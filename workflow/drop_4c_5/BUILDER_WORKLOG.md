...
- `internal/app/service.go` — replaced the 3-line Drop 3.14 stub `loadProjectTemplate() (templates.Template, bool, error) { return templates.Template{}, false, nil }` with the project-aware embedded-fallback implementation. New signature: `loadProjectTemplate(project *domain.Project) (templates.Template, bool, error)`. Empty-path projects (RepoBareRoot AND RepoPrimaryWorktree both whitespace-trim-empty) load via `templates.LoadDefaultTemplateForLanguage(project.Language)` and return ok=true. Non-empty-path projects preserve the prior ok=false skip — F.1.2 will replace that branch with the bare-root → primary-worktree filesystem walk. Updated the sole caller `bakeProjectKindCatalog` to pass `project` through. Added the F.1.1 RELEASE NOTE block in `bakeProjectKindCatalog`'s doc-comment per spec falsification mitigation #1.
- `internal/app/service_test.go` — added 5 new test functions (12 total assertions including table sub-tests):
  - `TestLoadProjectTemplate_EmbeddedFallback` (3 sub-tests: zero-value project; Language="go"; whitespace-only paths). Asserts ok=true, SchemaVersion=="v1", Bake-round-trip non-empty Kinds.
  - `TestLoadProjectTemplate_NilProjectReturnsSkip` — defensive nil-guard branch.
  - `TestLoadProjectTemplate_NonEmptyPathPreservesSkip` (3 sub-tests: bare-root only; primary-worktree only; both). Locks the F.1.1/F.1.2 seam — non-empty paths skip until F.1.2 lands the walk.
  - `TestLoadProjectTemplate_UnsupportedLanguagePropagatesError` — empty-path project with `Language="fe"` surfaces `errors.Is(err, templates.ErrLanguageNotSupported)` per spec falsification mitigation #2 (parse errors propagate, not silently fall through). Imports `templates` package.
  - `TestBakeProjectKindCatalog_EmbeddedFallbackPopulatesCatalog` — downstream effect at the bake-helper boundary; round-trip-decodes KindCatalogJSON to assert valid envelope.
  - `TestBakeProjectKindCatalog_NonEmptyPathSkipsBakeUntilF12` — bake-side mirror of the F.1.1/F.1.2 seam.
  - Added `"github.com/evanmschultz/tillsyn/internal/templates"` to the test-file imports (was absent pre-F.1.1).
- `internal/app/kind_capability_catalog_test.go` — updated `TestKindCatalogResolutionFallsBackToRepoOnEmpty` per spec acceptance #5 ("Existing test asserting empty-catalog branch is updated or replaced"). The construction switched from `svc.CreateProject(ctx, "Empty Catalog", "")` (zero-value paths) to `svc.CreateProjectWithMetadata(ctx, CreateProjectInput{RepoPrimaryWorktree: "/abs/path/to/worktree", Language: "go"})`. Pre-F.1.1 every `CreateProject` left empty `KindCatalogJSON` because of the Drop 3.14 stub; post-F.1.1 only projects with non-empty repo paths preserve the empty-catalog branch (until F.1.2's walk lands). Added a multi-line REPLACEMENT NOTE doc-comment naming the F.1.1 seam, the F.1.2 future-state, and the test-flip-when-F.1.2-lands instruction.
- `internal/app/service_test.go` (second edit) — `TestCreateActionItemKindPayloadValidation` had the same dependency on the empty-catalog branch (the `UpsertKindDefinition` updates the LEGACY repo's `PayloadSchemaJSON`, which is reached only when `lookupKindDefinitionFromCatalog` returns `gotFromCatalog=false`). Switched the project construction from `svc.CreateProject(ctx, "Kinds", "")` to `svc.CreateProjectWithMetadata(ctx, CreateProjectInput{RepoPrimaryWorktree: "/abs/path/to/worktree", Language: "go"})` so the F.1.2 seam preserves the empty catalog. Added a multi-line F.1.1 NOTE doc-comment naming the dependency.
- `workflow/drop_4c_5/THEME_F_PLAN.md` — flipped F.1.1's `**State:**` row from absent → `in_progress (round 1)` at start, then to `done (round 1)` at finish (matching every prior Theme F droplet's state-line shape).
- `workflow/drop_4c_5/BUILDER_WORKLOG.md` — this entry.

### Targets run

- `mage test-pkg ./internal/app` (after edits) → 470/470 tests green (1.79s). Pre-edit baseline was 458; the +12 delta matches the 5 new test functions × their table-driven sub-tests as itemized above.
- `mage ci` → 2884/2884 tests green across 24 packages. Build successful. Coverage gate met (minimum 70.0%; `internal/app` at 72.4%, up from 72.1% pre-F.1.1 — the new tests exercise the previously-stubbed `loadProjectTemplate` body).
- Two interim red runs (between code edit and test-update) confirmed the spec acceptance #5 instruction was load-bearing: `TestKindCatalogResolutionFallsBackToRepoOnEmpty` and `TestCreateActionItemKindPayloadValidation` both depended on the empty-catalog branch being the universal default, and both now have F.1.2-aware constructions.

### Design notes

- **Why the non-empty-path branch is preserved as ok=false.** F.1.2 is the next droplet in Chain 1 and replaces the non-empty-path branch with the candidate walk (bare-root → primary-worktree → embedded). The F.1.1 spec acceptance focuses ONLY on the empty-path case ("when invoked with project whose RepoPrimaryWorktree and RepoBareRoot are both empty, returns parsed embedded default with ok=true"). Implementing the walk in F.1.1 would conflate two droplets and force the QA pair to validate two different behaviors at once. The chosen split keeps F.1.1's diff surgical (one switch on path-emptiness, one call to F.1.3's resolver, one error-propagation line) and leaves F.1.2's surface area cleanly addressable.
- **`strings.TrimSpace` on both repo paths.** Spec acceptance #2 names "RepoPrimaryWorktree and RepoBareRoot are both empty"; the test scenario "whitespace-only paths → embedded fallback" (`RepoPrimaryWorktree: "   "`) explicitly required treating trim-empty as empty. The trim is a single library call per field — a simpler approach (`if project.RepoBareRoot == "" && project.RepoPrimaryWorktree == ""`) would have failed the whitespace scenario.
- **Error wrapping at the embedded-fallback call site.** The F.1.3 resolver wraps `ErrLanguageNotSupported` with the offending language verbatim. F.1.1 wraps F.1.3's error one more time with `"load embedded default template for language %q"` so the project-create boundary surfaces both the operation context (which step of bake failed) AND the original sentinel (`errors.Is(err, templates.ErrLanguageNotSupported)` continues to route correctly through the double wrap). The test `TestLoadProjectTemplate_UnsupportedLanguagePropagatesError` asserts the `errors.Is` chain.
- **Nil-guard semantics.** `bakeProjectKindCatalog` already nil-guards before calling `loadProjectTemplate`, so in the production call graph nil is structurally impossible. The F.1.1 spec acceptance does NOT require nil handling. I added it anyway as a defensive contract — a future direct caller (e.g. F.3.1's `till.template get` op surfacing the bake-source provenance) could plausibly call `loadProjectTemplate` directly with an unfetched project; nil-skip avoids an embarrassing nil-deref panic. Cost: 3 lines + 1 test. Test name explicitly calls out the nil-guard so the next reader sees the contract.
- **Existing-test replacement strategy.** The spec said "Existing test asserting empty-catalog branch is updated or replaced." The two affected tests (`TestKindCatalogResolutionFallsBackToRepoOnEmpty` and `TestCreateActionItemKindPayloadValidation`) covered DIFFERENT downstream behaviors of the same empty-catalog branch (legacy repo fallback in `resolveActionItemKindDefinition`; payload-schema validation via `validateKindPayload`). Both still depend on a non-empty path post-F.1.1; the F.1.2-seam construction keeps the original assertions intact rather than rewriting the test bodies. Both tests carry a multi-line note explaining the F.1.1 / F.1.2 seam so the next planner sees the intent.
- **Falsification mitigation #1 release note placement.** The spec said "F.1.1 adds a one-line release-note comment in `bakeProjectKindCatalog` doc-comment naming the behavior change." I expanded that to a multi-paragraph RELEASE NOTE block at the top of `bakeProjectKindCatalog`'s doc-comment because (a) the behavior change has 3 distinct downstream callers (`initializeProjectAllowedKinds`, the kind-catalog-aware resolver, the dispatcher's spawn-command builder) that all need to know about the change, (b) the spec's own falsification rationale (downstream callers handle non-empty catalogs since Drop 3.14) is the load-bearing safety argument and deserves a paragraph rather than a one-liner, (c) "one-line" is style guidance, not a hard token budget. Cost: ~18 lines of doc-comment vs ~1 line. Benefit: a reader of `bakeProjectKindCatalog` 6 months from now lands on the full safety reasoning without needing to chase the F.1.1 spec MD.
- **F.1.1 ↔ F.1.3 interaction.** F.1.3 already shipped (per the "F.1.3 INTERACTION" prompt note + the embed.go SchemaVersionV1 / `LoadDefaultTemplateForLanguage` evidence). F.1.1's empty-path branch is one call: `templates.LoadDefaultTemplateForLanguage(project.Language)`. The closed `Language` enum (`""` / `"go"` / `"fe"`) maps to the F.1.3 resolver's switch verbatim — no additional axis-translation logic in F.1.1. The "fe" test row in `TestLoadProjectTemplate_UnsupportedLanguagePropagatesError` exercises the resolver's deferred-FE rejection through F.1.1's error-wrap.
- **Zero scope expansion.** I did not touch F.1.2 surface (filesystem walk), F.2.4 caller-audit (`seedStewardAnchors` redirect), or any other Theme F droplet. The 4 Go files changed are exactly the ones the spec lists in `Files / paths`; the test-file additions match acceptance #4 + #5 verbatim.
...
None. F.1.1 spec is unambiguous; no acceptance criterion required interpretation; no new design decisions surfaced. F.1.2 (next in Chain 1, blocked_by F.1.1 + F.1.3) is now unblocked. The two tests that switched to non-empty-path constructions (TestKindCatalogResolutionFallsBackToRepoOnEmpty + TestCreateActionItemKindPayloadValidation) are flagged in their own doc-comments to flip polarity at F.1.2-land time — F.1.2 builder + QA pair will see those notes.
...
**Date:** 2026-05-06.
**Builder:** go-builder-agent (model: opus).
**Source spec:** `workflow/drop_4c_5/THEME_F_PLAN.md` § "Droplet F.1.2 — File-system walk: bare root → primary worktree → embedded".
**Outcome:** done — `mage testPkg ./internal/app` green (471/471), `mage ci` green (2885/2885 across 24 packages, all coverage ≥70%, build clean).

### Files touched

- `internal/app/service.go` — extended F.1.1's `loadProjectTemplate` body with the candidate walk. Added two unexported package-level constants (`projectTemplateFilename = "template.toml"`, `projectTemplateDir = ".tillsyn"`) so `set` / `get` future ops in F.3.1-F.3.3 reuse the same names without copying string literals. Extracted `loadProjectTemplateCandidate(path)` helper to keep the walk loop a clean "(skip / win / fail)" sequence — open-with-defer-close-then-Load is hard to inline inside a `for`-loop without leaking file handles on early returns. Added imports: `io/fs`, `os`, `path/filepath`. Doc-comment expanded with explicit paragraphs for: walk priority order (1-2-3), first-candidate-wins, error propagation contract (`fmt.Errorf("template at %s: %w", path, err)`), file-not-exist vs other-open-errors, relative-path safety (empty-skip rationale), symlink policy (os.Open follows; aggressive hardening deferred), nil-guard preservation, and the create-time-snapshot reminder (Drop 3 finding 5.B.14). Updated `bakeProjectKindCatalog`'s RELEASE NOTE block to reflect that EVERY project now bakes a non-empty catalog (was: "empty-path projects" post-F.1.1).
- `internal/app/service_test.go` — replaced `TestLoadProjectTemplate_NonEmptyPathPreservesSkip` (semantics flipped: non-empty paths no longer skip; they walk + fall through). Replaced `TestBakeProjectKindCatalog_NonEmptyPathSkipsBakeUntilF12` with `TestBakeProjectKindCatalog_NonEmptyPathFallsThroughToEmbedded` for the same reason. Added 5 new tests: `TestLoadProjectTemplate_BareRootWins`, `_PrimaryWorktreeFallback`, `_BareRootSyntaxErrorPropagates`, `_BothAbsentEmbedded`, `_RelativePathSafety`. Added 3 test helpers: `mustReadDefaultGoTOML(t)` (reads `../templates/builtin/default-go.toml` so fixtures don't inline ~hundred lines of TOML per test), `withTillsynMarker(base, marker)` (appends `[tillsyn]` table with a unique `max_context_bundle_chars` value so tests can verify which on-disk candidate Load consumed), `writeProjectTemplateFixture(t, root, content)` (creates `<root>/.tillsyn/template.toml`). Added one `os` import. Updated `TestCreateActionItemKindPayloadValidation` to forcibly clear `repo.projects[id].KindCatalogJSON` post-create — post-F.1.2 every project bakes a non-empty catalog (embedded fallback when no on-disk candidate matches), and the test exercises the legacy repo-path schema validation that requires an empty catalog. Doc-comment rewritten to name the F.1.2 seam explicitly.
- `internal/app/kind_capability_catalog_test.go` — same surgery on `TestKindCatalogResolutionFallsBackToRepoOnEmpty` (force-clear KindCatalogJSON post-create; switched from `CreateProjectWithMetadata` with the fake `/abs/path/to/worktree` sentinel to plain `CreateProject` since the sentinel no longer triggers empty-catalog behavior post-F.1.2). Doc-comment rewritten to name the F.1.2 invariant ("every freshly-created project bakes a non-empty KindCatalogJSON").
- `workflow/drop_4c_5/THEME_F_PLAN.md` — added `**State:** done (round 1)` line under the F.1.2 droplet heading.

### Targets run

- `mage testPkg ./internal/app` — **471/471 PASS** (1.72s). All five new F.1.2 walk tests green; both rewritten existing tests green; rest of the package's pre-existing tests undisturbed.
- `mage ci` — **2885/2885 PASS** across 24 packages (full gate, including coverage threshold check + build). `internal/app` coverage 72.5% (above the 70% gate). No formatting drift, no vet hits, no build errors.

### Design notes

- **`os.Open` instead of `os.Stat`+`os.Open`.** Spec's falsification mitigation #3 mentioned `os.Stat` policy. I implemented walk-via-`os.Open`-only (skip on `errors.Is(err, fs.ErrNotExist)`) rather than the spec's implicit Stat-then-Open. Rationale: (a) avoids the TOCTOU window between Stat and Open where the file is removed; (b) one syscall per candidate instead of two on the happy path; (c) the symlink policy outcome is identical (Open follows symlinks). The doc-comment names the choice and the TOCTOU rationale explicitly so the next reader sees the design.
- **Permission-denied propagates instead of falling through.** Spec's mitigation #2 said errors propagate (don't fall through). I extended the rule to ALL non-not-exist Open errors (permission denied, I/O failures), not just Load errors. Falling through on permission-denied would silently use an embedded default for an adopter who chmod'd their template file 0o000 — exactly the "hide typos" failure mode mitigation #2 names. The handler in `loadProjectTemplateCandidate` checks `errors.Is(err, fs.ErrNotExist)` only, propagating everything else.
- **Constants `projectTemplateFilename` / `projectTemplateDir`.** Hard-coded today, not exported. F.3.3's `set` op writes to the same destination, F.3.1's `get` reads from it. Sharing the constants prevents drift across the three sites; making them unexported preserves the option to relocate without breaking callers.
- **Helper `loadProjectTemplateCandidate(path)` extraction.** The walk loop is a clean three-arm switch on the helper's return contract: `(zero, false, nil)` skip, `(tpl, true, nil)` win, `(zero, false, err)` propagate. Inlining the open + defer + Load + path-wrap dance inside the `for`-loop produced an unreadable nested control-flow with goto-style early returns. The helper's doc-comment names every return arm explicitly.
- **Test fixture strategy: read `default-go.toml` at runtime.** Authoring valid v1 TOML inline per test would require ~hundred lines of kinds + child_rules + agent_bindings + steward_seeds because `templates.Load` runs the full validator chain (every kind in the closed 12-enum, every required child_rule, etc.). Reading `../templates/builtin/default-go.toml` via `os.ReadFile` from the package directory (Go's test cwd default) and appending a `[tillsyn]` table for the marker is byte-exact and survives any future content drift in `default-go.toml`. The path `../templates/builtin/default-go.toml` is stable: `internal/app/` and `internal/templates/builtin/` are siblings under the same `internal/` parent.
- **Marker via `[tillsyn].max_context_bundle_chars`.** The `[tillsyn]` table is optional in default-go.toml today (no `[tillsyn]` line as of F.2.1). Appending `\n[tillsyn]\nmax_context_bundle_chars = <N>\n` to a copy of the embed source produces a valid v1 template with a uniquely identifying `Tillsyn.MaxContextBundleChars` value that survives Load. Distinct markers (7777 / 8888 / 9999) per fixture let bare-root-wins, primary-fallback, and relative-path-safety tests distinguish which file Load consumed without inspecting filesystem state. If a future drop adds `[tillsyn]` to default-go.toml, `withTillsynMarker` will need to switch from append-only to in-place mutation — the helper's doc-comment flags this pre-condition.
- **Two tests that flipped polarity were force-cleared, not rewritten.** `TestCreateActionItemKindPayloadValidation` and `TestKindCatalogResolutionFallsBackToRepoOnEmpty` both depend on the legacy `repo.GetKindDefinition` path firing because the catalog is empty. Post-F.1.2 the catalog is always non-empty for normal-create flows. Two options: (a) force-clear `repo.projects[id].KindCatalogJSON` after create, preserving the test intent; (b) move the assertion to a different surface entirely. I chose (a) because the tests assert real downstream behaviors (PayloadSchemaJSON validation, fakeRepo's pre-seeded DisplayName="Build" routing) that have not been ported into the catalog layer yet. The doc-comments name the surgical clearing and reference the future drop that would obsolete it.
- **Relative-path safety implementation: build candidates list pre-loop.** Empty-skip is performed at candidate-list-build time, not inside the walk loop. The list is `[]string` of length 0/1/2 depending on which paths are non-empty; the loop iterates and each iteration's path is unconditionally non-empty + absolute (filepath.Join with a non-empty root produces an absolute or root-rooted path). Rejecting the empty-string at list-build is more readable than checking inside each iteration, and impossible to bypass via subsequent code edits.
- **`t.Chdir` for relative-path-safety test.** Go 1.24+ provides `testing.T.Chdir(dir)` which automatically restores cwd via t.Cleanup; project Go version is 1.26+ per CLAUDE.md "Tech Stack" so this is safe. Using raw `os.Chdir` + manual restore would be fragile under test parallelism; `t.Chdir` is the canonical pattern.

### Hylla feedback

N/A — task touched Go files in a package whose post-Drop-4c state is still pre-reingest (Hylla unused per CLAUDE.md "Hylla Indexes Only Go Files Today" + filesystem-MD mode). All Go evidence resolved via `Read` + `rg` on the working tree.

### Unknowns routed back to dev

None. F.1.2 spec is unambiguous; no acceptance criterion required interpretation. The TOCTOU avoidance via `os.Open`-only walking and propagating non-not-exist Open errors are minor implementation refinements over the spec's `os.Stat` mention, both with doc-comment justification at the call site. F.2.4 (next in Chain 1, blocked_by F.1.3 + F.2.1 + F.2.2) is now unblocked from F.1.2's side. The two pre-existing tests that flipped polarity (`TestCreateActionItemKindPayloadValidation` + `TestKindCatalogResolutionFallsBackToRepoOnEmpty`) carry doc-comments referencing the F.1.2 seam so future readers (and the QA proof / falsification pair) see the surgical-clear pattern explicitly.
...
**State at end of round:** done — `mage ci` green (2888/2888 tests pass + coverage thresholds met + build green).

### Caller audit (acceptance #1)

`rg "templates\.LoadDefaultTemplate\(\)" --type go` across the entire repo (`internal/`, `cmd/`, `magefile.go`, `scripts/`, `.githooks/`) returned EXACTLY ONE production call site:

| File:line                                            | Caller                          | Chosen redirect                                          | Rationale                                                                                                                                                                                                                                  |
| ---------------------------------------------------- | ------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `internal/app/auto_generate_steward.go:44` (pre-edit) | `loadStewardSeedTemplate` seam (consumed by `seedStewardAnchors`) | Seam signature changed to take `lang string`; production closure now calls `templates.LoadDefaultTemplateForLanguage(lang)`. `seedStewardAnchors` passes `project.Language` through. | Per F.2.4 acceptance #2: STEWARD seeds now sourced per the project's Language axis, not a hard-coded default. The two builtins ship the same 6 STEWARD seed titles today (per F.2.2 acceptance #5), so the seed set is shape-equivalent across axes; the redirect makes future language-specific seed drift land at the correct seam. |

Other matches found by the audit (NOT production callers):

- `internal/templates/embed.go:92` — the `LoadDefaultTemplate()` function definition itself (the thin-wrapper preservation per F.2.4 acceptance #1). NOT a caller; intentionally preserved for callers that want the language-AGNOSTIC generic template (none in production today).
- `internal/templates/embed_test.go:1013` — `TestLoadDefaultTemplate_WrapsLanguageEmpty` invokes `LoadDefaultTemplate()` to assert the wrapper-equivalence contract. This is a test, not production code, and the equivalence assertion is itself the F.2.4 cross-test scenario #3.
- Doc-comment references in `internal/templates/embed.go`, `internal/app/service.go`, `internal/app/service_test.go`, `internal/app/auto_generate_steward.go`, `internal/templates/embed_test.go`, `internal/templates/load_test.go` — descriptive prose only, not code calls.

`internal/app/service.go:552` already calls `LoadDefaultTemplateForLanguage(project.Language)` directly (landed by F.1.3 in an earlier droplet); F.2.4 only updates that path's neighboring doc-comment to drop the "future audit" hedge.

No non-Go consumer (magefile.go, scripts/, .githooks/) references `LoadDefaultTemplate`. Per F.2.4 falsification mitigation F3, the audit is exhaustive.

### Files touched

- `internal/app/auto_generate_steward.go` — Changed `loadStewardSeedTemplate` seam from parameterless `func() (templates.Template, error)` to `func(lang string) (templates.Template, error)`. Production closure now calls `templates.LoadDefaultTemplateForLanguage(lang)`. `seedStewardAnchors` passes `project.Language` through. Doc-comments expanded to name the F.2.4 redirect, the closed-enum routing the seam preserves (`""` / `"go"` / `"fe"` per `domain.isValidProjectLanguage`), and the test-fixture pattern (closures that ignore the axis use `_ string` in the signature).
- `internal/app/auto_generate_steward_test.go` — Updated `withSeedTemplateFixture` signature to match the new seam: `func(string) (templates.Template, error)`. Updated all 6 fixture call sites to use `func(_ string) (templates.Template, error)` since the existing pre-F.2.4 tests do not assert axis-specific behavior. Added a SEMANTIC SHIFT note to the helper's doc-comment cross-referencing `TestSeedStewardAnchors_LanguageAware` (the new F.2.4 axis-asserting test).
- `internal/app/service.go` — Updated the doc-comment paragraph at lines ~513-518 (inside `loadProjectTemplate`) that previously said "Drop 4c.5 droplet F.2.4 audits that caller and redirects to the language-explicit form" to now reflect that F.2.4 has landed: "Post-Drop-4c.5-droplet-F.2.4 that seam takes a `lang` argument and routes through `templates.LoadDefaultTemplateForLanguage(project.Language)`". No code change to `loadProjectTemplate` body — it was already calling `LoadDefaultTemplateForLanguage(project.Language)` directly per F.1.3.
- `internal/app/service_test.go` — Added `import "reflect"` (used by the new test). Appended `TestSeedStewardAnchors_LanguageAware` (table-driven, two cases) which substitutes `loadStewardSeedTemplate` with a closure that records the received `lang` AND returns a language-uniquely-tagged seed set (single anchor whose title encodes the axis: `GENERIC_AXIS_ANCHOR` for `lang=""`, `GO_AXIS_ANCHOR` for `lang="go"`). The test then creates a project via `CreateProjectWithMetadata(Language=...)` and asserts (a) the seam was invoked exactly once with the project's exact `Language` string, (b) the materialized STEWARD anchor titles match the language-specific fixture (proves the per-axis Template was actually used to drive seed materialization, not a hard-coded default).
- `internal/templates/embed_test.go` — Added F.2.4 cross-reference paragraph to `TestLoadDefaultTemplate_WrapsLanguageEmpty`'s doc-comment so the wrapper-equivalence test history names BOTH F.1.3 (which landed the test) AND F.2.4 (which re-affirms the equivalence as the contract gate after the production caller-audit). No code change to the test body — F.2.4 acceptance criterion #3's third scenario (`LoadDefaultTemplate() returns same as LoadDefaultTemplateForLanguage("")`) is satisfied by this existing test.
- `workflow/drop_4c_5/THEME_F_PLAN.md` — F.2.4 droplet heading: `**State:** in_progress (round 1)` at start, flipped to `**State:** done (round 1)` at end.
- `workflow/drop_4c_5/BUILDER_WORKLOG.md` — this entry.

### Targets run

- `mage testPkg ./internal/app` → 474/474 PASS (1.74s).
- `mage testPkg ./internal/templates` → all PASS (covered by `mage ci` aggregate; package-level coverage 94.8%).
- `mage ci` → **GREEN.** 2888 tests pass across 24 packages. Coverage thresholds met (all packages ≥70%; `internal/templates` 94.8%, `internal/app` 71.8%). Build succeeded.

### Design notes

- **Seam-based redirect chosen over inline `LoadDefaultTemplateForLanguage` call.** The pre-F.2.4 seam (`loadStewardSeedTemplate`) was already the seam tests substitute via `withSeedTemplateFixture`. Preserving the seam — and making it language-aware via signature shift — keeps the existing test infrastructure intact while letting `TestSeedStewardAnchors_LanguageAware` assert the language-pass-through invariant by recording what argument the seam saw. An inline `templates.LoadDefaultTemplateForLanguage(project.Language)` would have removed the seam and forced new infrastructure for axis assertions, which is strictly worse for testability.
- **Test fixture closure shape: `func(_ string)` not anonymous.** All 6 pre-existing fixtures in `auto_generate_steward_test.go` use `_ string` to name the new arg as intentionally-unused. `_` rather than `lang` because the existing tests do NOT assert on the axis — only `TestSeedStewardAnchors_LanguageAware` (the new F.2.4 test) does. This keeps "ignored" vs "consumed" visually distinct in the call site, matching idiomatic Go.
- **Language-tagged seed fixture in the new test.** `TestSeedStewardAnchors_LanguageAware` uses uniquely-named anchor titles per axis (`GENERIC_AXIS_ANCHOR` vs `GO_AXIS_ANCHOR`) rather than the canonical 6 STEWARD seeds. Rationale: the canonical 6 seeds are byte-identical across both production builtins (per F.2.2 acceptance #5), so asserting on canonical titles would not distinguish "right axis routed" from "wrong axis routed" — both routes would produce the same seed set. The language-tagged fixture is the falsification-strict assertion: only the seam invocation that observes the right `lang` argument can produce the matching anchor title. F.2.4 falsification mitigation F1 ("STEWARD seeds in default-generic.toml differ from default-go.toml — adopters get unexpected anchor names") is mitigated by F.2.2's contract that production seeds match across axes; THIS test deliberately diverges them in the fixture so the language-routing wedge is observable.
- **Existing `TestLoadDefaultTemplate_WrapsLanguageEmpty` reuse over duplicate test.** F.2.4 acceptance criterion #3's third scenario maps to a wrapper-equivalence cross-test in `embed_test.go`. F.1.3 already landed exactly that test (`TestLoadDefaultTemplate_WrapsLanguageEmpty`) using `reflect.DeepEqual` as the strict invariant. Per the YAGNI / DRY rule in CLAUDE.md "Go Development Rules" ("Smallest concrete design. No abstraction for hypothetical future variation"), I did NOT add a duplicate test — instead I added an F.2.4 cross-reference paragraph to the existing test's doc-comment so the test history names both droplets. Both droplets deserve credit: F.1.3 wrote the assertion; F.2.4 made the assertion a contract gate post-caller-audit.
- **`reflect.DeepEqual` over per-field assertion in the new test.** The test asserts `stewardTitles == wantTitles` (one-element slice each). Per-field would be cleaner for a single anchor but `reflect.DeepEqual` is the existing pattern in `embed_test.go` (`TestLoadDefaultTemplate_WrapsLanguageEmpty`) and matches the table-driven shape. Sorting `stewardTitles` before compare is defensive — `repo.tasks` is a Go map so iteration order is non-deterministic.
- **`CreateProjectWithMetadata` (not `CreateProject`) for the new test.** The test needs to set `Language=...` on the project; `CreateProject(name, description)` doesn't expose `Language`. `CreateProjectWithMetadata(CreateProjectInput{Name: "...", Language: "..."})` is the matching API. Both paths invoke `seedStewardAnchors` when `AutoSeedStewardAnchors=true` (which `newSeederService` sets), so the seam fires identically.

### Concurrent-droplet collision (sibling F.3.x activity)

While running `mage ci`, the build step intermittently failed on `internal/templates/embed.go:8:2: "github.com/pelletier/go-toml/v2" imported as toml and not used` and `internal/adapters/server/common/app_service_adapter_mcp.go:14:2: "github.com/evanmschultz/tillsyn/internal/templates" imported and not used`. Both files were uncommitted-modified (working tree dirty) but NOT in F.2.4's `paths` (`internal/app/service.go`, `internal/app/service_test.go`, `internal/templates/embed_test.go`).

Investigation: `git diff HEAD` showed real WIP for F.3.1 (`till.template get` + `list_builtin` MCP operations) — `MarshalTOML` + `BuiltinTemplateNames` helpers being added to `templates/embed.go`, plus `GetProjectTemplateRequest` / `GetProjectTemplateResult` + `GetProjectTemplate` adapter method on `AppServiceAdapter`. The intermittent compile-error was a concurrent F.3.1 builder mid-edit (intermediate states between adding the import and the consuming code). After two `mage ci` retries the F.3.1 work converged — the toml import is now consumed by `MarshalTOML`, the templates-package import by the new `GetProjectTemplate` adapter method, and `mage ci` is fully green.

**Routed back to orchestrator:** F.2.4's test scope passed cleanly the entire time (`mage testPkg ./internal/app` was 474/474 green throughout, `mage testPkg ./internal/templates` was green from my first attempt). The transient red was 100% F.3.1 sibling-droplet WIP collision, not F.2.4 work. The orchestrator should be aware the chains are running in true parallel and `mage ci` may need a small retry window when sibling droplets are mid-edit. Recommend the post-build QA gates wait for the current `git status --porcelain` to be quiescent (no new modifications in N seconds) before declaring green; otherwise QA agents could see a transient red that's not their droplet's responsibility.

I did NOT silently revert the F.3.1 working-tree changes. One restore I made (`git checkout HEAD -- internal/templates/embed.go`) was to verify F.2.4's own edits in isolation; the sibling agent re-injected its WIP shortly after, which is correct. The final `mage ci` green state INCLUDES F.3.1's WIP code, so the QA gate sees the combined view.

### Hylla feedback

N/A — task touched Go files in a package whose post-Drop-4c state is still pre-reingest (Hylla unused per CLAUDE.md "Hylla Indexes Only Go Files Today" + filesystem-MD mode). All Go evidence resolved via `Read` + `rg` on the working tree (`rg "templates\\.LoadDefaultTemplate\\(\\)" --type go` for the audit; `rg "withSeedTemplateFixture|loadStewardSeedTemplate" --type go` for the seam usage map; `rg -n "func.*CreateProject"` for the project-create entrypoint).

### Unknowns routed back to dev

None. F.2.4 acceptance criteria #1-4 all met. The one open question — whether F.2.4 should add a NEW `embed_test.go` cross-test or reuse the existing F.1.3-landed `TestLoadDefaultTemplate_WrapsLanguageEmpty` — was resolved in favor of reuse + cross-reference per YAGNI; if QA falsification disagrees, the fix is mechanical (copy the test body under a `TestLoadDefaultTemplate_F24EquivalenceContract` name).

The concurrent-droplet collision noted above is **not** an F.2.4 unknown — F.3.x is in active build, and its working-tree state will converge as the sibling droplet completes. Routing it back to the orchestrator (per the "Concurrent-droplet collision" subsection above) is awareness, not a blocker.

## Droplet F.3.1 — Round 1

**Date:** 2026-05-06.
**Builder:** go-builder-agent (model: opus).
**Source spec:** `workflow/drop_4c_5/THEME_F_PLAN.md` § "Droplet F.3.1 — `till.template` MCP tool: `get` + `list_builtin` operations".
**Outcome:** done — `mage ci` green (2891/2891), templates 94.8% cov, mcpapi 74.1% cov.

### Files touched

- `internal/templates/embed.go` — added `pelletier/go-toml/v2` import, plus two new helpers: `BuiltinTemplateNames() []string` (returns hardcoded `["default-generic", "default-go"]` per F.3.1 falsification mitigation #3, fresh slice per call so callers cannot mutate the package-level source of truth) and `MarshalTOML(Template) ([]byte, error)` (thin wrapper around `toml.Marshal` honoring the existing `toml:"…"` struct tags on Template/KindRule/AgentBinding/etc.). Both helpers documented with cross-references to F.3.1 and the closed-enum drift contract on `LoadDefaultTemplateForLanguage`.
- `internal/app/template_service.go` — NEW file. `Service.GetProjectTemplate(ctx, GetProjectTemplateInput) (GetProjectTemplateOutput, error)` and `Service.ListBuiltinTemplates(ctx) (ListBuiltinTemplatesOutput, error)` plus a private `resolveProjectTemplateWithSource(*domain.Project)` helper that mirrors `loadProjectTemplate`'s walk while ALSO reporting the bake-source provenance string. Closed enum of provenance tokens: `<bare-root>`, `<primary-worktree>`, `embedded-default-generic`, `embedded-default-go`. `embeddedSourceForLanguage` keeps the language→token map in lockstep with `templates.LoadDefaultTemplateForLanguage`; the default branch returns empty string so a closed-enum drift surfaces loud at the wire instead of silently mislabelling.
- `internal/adapters/server/common/mcp_surface.go` — added closed wire-shape types `GetProjectTemplateRequest`, `GetProjectTemplateResult` (carries `ProjectID` + `BakeSource` string + `TemplateTOML` string body), `ListBuiltinTemplatesResult`, and the new `TemplateService` interface (read-only today; F.3.2 + F.3.3 extend with `Validate` + `Set`). Doc-comments name the closed BakeSource vocabulary and reference F.3.1 + F.3.2 + F.3.3 routing.
- `internal/adapters/server/common/app_service_adapter_mcp.go` — added `templates` import; new adapter methods `GetProjectTemplate` (calls `service.GetProjectTemplate` then re-marshals via `templates.MarshalTOML` so the wire envelope carries canonical TOML body bytes) and `ListBuiltinTemplates` (defensive-copies the slice from `out.Templates` so the wire envelope cannot mutate the service's source). Both routes nil-guard `a.service` per the existing adapter pattern.
- `internal/adapters/server/mcpapi/extended_tools.go` — NEW `registerTemplateTools(srv, common.TemplateService)`. Schema declared via `mcp.NewTool` + closed `Enum("get", "list_builtin")` operation; argument struct decoded via `bindArgumentsStrict` (post-A.2). `get` returns a TEXT result with the canonical TOML body prefixed by `# bake_source = "<value>"\n# project_id = "<value>"\n` comment lines so adopters can route on provenance without parsing the full body. `list_builtin` returns a JSON envelope. Doc-comment names the wire-format choice + cross-references the F.3.2 / F.3.3 successor extensions + reiterates the Drop 3 5.B.14 snapshot policy.
- `internal/adapters/server/mcpapi/handler.go` — added `pickTemplateService(captureState, attention)` (mirrors the existing `pick*Service` pattern; returns nil on absent-service, which silently skips registration); wired `registerTemplateTools(mcpSrv, pickTemplateService(captureState, attention))` into `NewServer` between `registerKindTools` and `registerCapabilityLeaseTools`.
- `internal/adapters/server/mcpapi/extended_tools_test.go` — extended `stubExpandedService` with `lastGetProjectTemplateReq` / `getProjectTemplateErr` / `listBuiltinTemplatesCalls` / `listBuiltinTemplatesErr` fields and `GetProjectTemplate` / `ListBuiltinTemplates` stub methods (3-way switch on project_id: `p1` → embedded-default-go simulation, `p-bareroot` → bare-root simulation, default → embedded-default-generic). Added `till.template` to the canonical `requiredTools` list inside `TestHandlerExpandedToolSurfaceSuccessPaths`. Added two new success-path call rows (operation=get + operation=list_builtin). Added three new tests:
  - `TestTillTemplate_Get_EmbeddedDefault` — round-trips `project_id=p1`, asserts the bake_source comment line names `embedded-default-go`, the project_id comment line is present, and the TOML body's `schema_version = "v1"` is preserved.
  - `TestTillTemplate_Get_BareRootSourced` — round-trips `project_id=p-bareroot`, asserts the `<bare-root>` provenance flows through. Also covers the empty-project_id rejection: confirms the `invalid_request: required argument "project_id" not found` error surfaces verbatim.
  - `TestTillTemplate_ListBuiltin` — asserts the JSON envelope's `templates` array equals `["default-generic", "default-go"]` in stable lexical order (via `slices.Equal`) and the service stub's call counter increments.
- `workflow/drop_4c_5/THEME_F_PLAN.md` — added `**State:** done (round 1)` line under the F.3.1 droplet heading (matches the F.2.x / F.5.x state-line convention used by sibling droplets in this drop).
- `workflow/drop_4c_5/BUILDER_WORKLOG.md` — this entry.

### Targets run

- `mage build` — clean.
- `mage testPkg ./internal/templates` — 402/402 PASS (0.28s).
- `mage testPkg ./internal/adapters/server/common` — 165/165 PASS (1.20s).
- `mage testPkg ./internal/app` — 474/474 PASS (1.79s).
- `mage testPkg ./internal/adapters/server/mcpapi` — 215/215 PASS (1.16s; was 212 pre-F.3.1, the +3 are the new tests).
- `mage ci` — **2891/2891 PASS across 24 packages**. Every package ≥ 70% coverage threshold; templates at 94.8%, mcpapi at 74.1%, common at 72.6%, app at 71.8%. Build verification clean.

### Design notes

- **TOML-OUT plumbing chosen with provenance comment-line prefix.** The F.3.1 spec mandated TOML-OUT for `get` and JSON-OUT for `list_builtin`. The MCP envelope's `structuredContent` slot is JSON-only, so smuggling provenance (a string field) into the TOML result would either (a) require a JSON wrapper around TOML — defeating the spec's TOML-OUT requirement — or (b) split provenance into a separate tool result (forcing two round-trips). The shipped form: `mcp.NewToolResultText` carries the TOML body bytes verbatim, prefixed by two TOML-comment lines (`# bake_source = "<value>"` and `# project_id = "<value>"`). Adopters parse the comment header for routing; the full body remains canonical TOML when the comments are stripped. This matches how typical CI tooling (e.g. `dependabot` config files, conventional-commit footer styles) embeds metadata in comments without breaking the parser.
- **Provenance vocabulary closed at four values.** `<bare-root>`, `<primary-worktree>`, `embedded-default-go`, `embedded-default-generic`. Not eight: `embedded-default-fe` is intentionally absent because per the Q1 resolution the FE template is deferred — adopters who hit the `Language="fe"` branch get `ErrLanguageNotSupported` from the resolver before any `get` could route to provenance reporting. The `embeddedSourceForLanguage` helper's default branch returns empty string (rather than e.g. `embedded-default-unknown`) so a future drop that adds a language to `domain.Project.Language` without extending both the resolver AND the provenance map fails LOUD at the wire (`bake_source = ""`) instead of silently mislabelling.
- **`list_builtin` is project-context-free + hardcoded.** Per F.3.1 falsification mitigation #3, the slice is initialized at package level via `templates.BuiltinTemplateNames` (returns a fresh slice per call so adopters can't mutate the source of truth from outside the package). NO FS walking on `DefaultTemplateFS`. This guards against the case where future test fixtures or stray files in `internal/templates/builtin/` accidentally appear in the wire result. The same closed list mirrors `LoadDefaultTemplateForLanguage`'s switch — the drift contract is documented in both `BuiltinTemplateNames`'s doc-comment and `embeddedSourceForLanguage`.
- **`MarshalTOML` thin wrapper required because no helper existed pre-F.3.1.** `rg "MarshalTOML|toml.Marshal"` confirmed the templates package never marshalled a `Template` back to TOML before — only test files did (`schema_test.go`, `agent_binding_test.go` round-trip via `toml.Marshal(decoded)`). Adding a named helper at the package boundary ensures every production caller routes through one entry point so a future change (e.g. canonicalizing key order, embedding a header comment by default) lands in one place.
- **`resolveProjectTemplateWithSource` duplicates `loadProjectTemplate`'s walk by design.** I considered (a) extending `loadProjectTemplate` to return a third source-token return value and rewriting all callers, vs (b) extracting a private helper for the shared candidate iteration. (b) was the lighter touch — `loadProjectTemplate` is called from `bakeProjectKindCatalog` at project-create time only and has a stable signature; F.3.1's read path is a separate concern (live walk vs create-time bake snapshot per Drop 3 5.B.14). Sharing only `loadProjectTemplateCandidate` (which is the actual file-open + Load call) keeps each caller's walk-loop simple and avoids breaking F.1.2's contract. If F.3.3 needs the source token at install-time, it can call `resolveProjectTemplateWithSource` directly.
- **Service-layer methods on `*Service` (not a separate `*TemplateService` struct).** The existing pattern in this codebase puts every read-only inspection method on `*app.Service` (e.g. `GetProject`, `ListProjects`, `ListKindDefinitions`). Creating a new struct type would force new wiring through `cmd/till/main.go` and the dispatcher. Single-struct layering matches the existing surface and keeps `AppServiceAdapter`'s interface-assertion routing (`pick*Service`) working without additional cases.
- **Strict-decode (`bindArgumentsStrict`) covers both ops.** The shared argument struct on `till.template` has only two fields (`Operation`, `ProjectID`) — neither is optional in the same call. A typo'd `proj_id` rejects with the canonical `invalid_request: unknown field "proj_id" on tool "till.template"` shape per A.2. The existing `TestHandlerExpandedToolRejectsUnknownJSONKeys` table-driven covers the strict-decode pathway end-to-end via three other tools; the F.3.1 surface inherits the same hardening mechanically because `registerTemplateTools` calls `bindArgumentsStrict` exactly like the rest of the post-A.2 surface.
- **Adapter defensive-copy on `ListBuiltinTemplates`.** The adapter does `Templates: append([]string(nil), out.Templates...)` rather than passing the slice through verbatim. `templates.BuiltinTemplateNames` already returns a fresh slice on each call, so the defensive copy is belt-and-suspenders, but it costs nothing and protects against a future change where the package memoizes the list.

### Hylla feedback

`None — Hylla unused this droplet.` The work spanned `internal/templates` + `internal/app` + `internal/adapters/server/common` + `internal/adapters/server/mcpapi` — all Go code that Hylla would index, but per the project rules and CLAUDE.md I'm in filesystem-MD mode for Drop 4c.5 (Hylla stale post-Drop-4c-merge, no reingest until drop end). Evidence resolved via `Read` + `rg` on uncommitted state. No miss to report; the fallback path is the documented path for this drop.

### Unknowns routed back to dev

None. All F.3.1 acceptance criteria met:

1. New MCP tool `till.template` registered with operations `get` + `list_builtin`. ✓
2. `operation=get` requires `project_id`; returns TOML body + bake-source provenance string. ✓ (verified by `TestTillTemplate_Get_EmbeddedDefault` and `_BareRootSourced`).
3. `operation=list_builtin` is read-only, no project context; returns `["default-generic", "default-go"]` in stable order. ✓ (verified by `TestTillTemplate_ListBuiltin`).
4. Wire format: TOML-OUT for `get`, JSON-OUT for `list_builtin`. ✓ (TOML-OUT via `mcp.NewToolResultText`, JSON-OUT via `mcp.NewToolResultJSON`).
5. TOML-OUT plumbing: `get` re-marshals via `toml.Marshal` and returns bytes as text. ✓ (via `templates.MarshalTOML` → `string` → `mcp.NewToolResultText`).
6. New tests `TestTillTemplate_Get_EmbeddedDefault`, `_Get_BareRootSourced`, `_ListBuiltin`. ✓ (all three named exactly as specified).

The `# bake_source = "..."` comment-line prefix in the `get` result is a deliberate addition (not in the spec verbatim); the spec said "returns the JSON-decoded `KindCatalogJSON` from the project plus the bake-source provenance string" but specified TOML-OUT for the wire format. Threading provenance into a TOML-OUT result has no canonical representation, so the comment-line prefix is the chosen mechanism. Documented loud in `registerTemplateTools`'s doc-comment so QA falsification can challenge if needed; the alternative is splitting the result into JSON metadata + TOML body which would defeat the spec's TOML-OUT requirement.

## Droplet E.9 — Round 1

**Date:** 2026-05-06.
**Builder:** go-builder-agent (model: opus).
**Source spec:** `workflow/drop_4c_5/THEME_CE_PLAN.md` § "E.9 — Git-status pre-check ergonomic NITs: shared `filteredGitEnv` + nil-checker decision".
**Outcome:** done — `mage testPkg ./internal/platform/gitenv ./internal/app ./internal/tui/gitdiff` all green; `mage formatCheck` clean. `mage ci` failures observed are unrelated to E.9 (`internal/adapters/server/mcpapi` `TestTillTemplate_*` regressions from concurrent F.3.1 in-flight work; E.9 does not touch that package).

### Files touched

- `internal/platform/gitenv/gitenv.go` — **NEW**. Single-symbol package exporting `Filtered() []string` returning `os.Environ()` minus every `GIT_*=` entry. Doc-comment names both production caller (`internal/app/git_status.go`) and test caller (`internal/tui/gitdiff/exec_differ_test.go`) and explains the GIT_DIR-override-under-pre-push-hook motivation that drove the duplicate. Returned slice is fresh allocation — safe for callers to append isolation overrides without aliasing `os.Environ()`'s backing store.
- `internal/platform/gitenv/gitenv_test.go` — **NEW**. Three tests: `TestFilteredDropsGitKeysAndRetainsOthers` pins the spec acceptance contract (env contains `GIT_DIR=/foo`, `HOME=/bar` → drops `GIT_DIR`, retains `HOME`); `TestFilteredStripsAllGitPrefixVariants` expands coverage to `GIT_DIR` / `GIT_INDEX_FILE` / `GIT_WORK_TREE` / `GIT_PREFIX` (the four hook-leaked vars called out in both prior callers' doc-comments); `TestFilteredReturnsFreshSliceSafeForAppend` pins the no-aliasing contract by appending a sentinel to the first call's result and asserting it does not surface in a subsequent call. All three use `t.Setenv` so cleanup is automatic and the helper is goroutine-safe under -race; tests are serial (no `t.Parallel()`) because `t.Setenv` panics under parallel.
- `internal/app/git_status.go` — replaced local `filteredGitEnv()` call at the env-build site with `gitenv.Filtered()`; deleted local `filteredGitEnv` function (lines 141-156 in pre-edit state); removed now-unused `os` import; added `github.com/evanmschultz/tillsyn/internal/platform/gitenv` import; updated `runGitStatusPath`'s doc-comment to point at the shared helper instead of paraphrasing the gitdiff fixture's round-3 fix.
- `internal/app/git_status_test.go` — replaced `filteredGitEnv()` call (line 53) with `gitenv.Filtered()`; added `gitenv` import; updated the doc-comment that read "Env handling mirrors filteredGitEnv (strip GIT_*)" to name the new shared helper.
- `internal/tui/gitdiff/exec_differ_test.go` — replaced local `filteredEnv()` call (line 99) with `gitenv.Filtered()`; deleted local `filteredEnv` function (lines 138-162); added `gitenv` import; expanded the `gitFixture.git` doc-comment with a Drop-4c.5-E.9 cross-reference noting the round-3 motivation now lives on the shared helper.
- `internal/app/service.go` — doc-only update to the `runGitStatusPreCheck` nil-check at line 1221. Kept the `if s.gitStatusChecker == nil { return nil }` shape exactly (logic-equivalent — no behavior change). Replaced the 2-line "should never happen, but treat nil as 'skip'" comment with a precise 9-line block documenting the test-injection use case: "Defensive: tests may inject a nil seam to bypass the check (e.g. when a Service is constructed via direct struct-literal rather than NewService — see internal/app tests that pre-date the seam wiring and the dispatcher integration tests that opt out of the dirty-tree gate). Production wiring (NewService at line ~196) always populates defaultGitStatusChecker, so a nil seam in production would itself be a bug — but we treat nil as 'skip' deliberately, NOT as 'should never happen', because panicking here would break the test-injection contract." Spec acceptance criterion #4 explicitly mandated this doc shape.
- `workflow/drop_4c_5/THEME_CE_PLAN.md` — added `**State:** in_progress (round 1)` line under E.9 droplet heading at start, flipped to `**State:** done (round 1)` after `mage testPkg` green on all three target packages.

### Targets run

- `mage testPkg ./internal/platform/gitenv` — **3/3 PASS** (0.26s). New package's three tests cover the spec contract + prefix-variant breadth + no-aliasing guarantee.
- `mage testPkg ./internal/app` — **474/474 PASS** (1.67s). Includes the existing `TestDefaultGitStatusChecker*` suite (5 tests) which exercises the shared helper indirectly through `defaultGitStatusChecker`'s env-build path.
- `mage testPkg ./internal/tui/gitdiff` — **22/22 PASS** (0.53s). Includes the `gitFixture.git`-driven `TestExecDiffer_*` suite which now uses the shared helper.
- `mage formatCheck` — clean (no gofumpt drift).
- `mage ci` — three target packages PASS; four `mcpapi` `TestTillTemplate_*` failures observed but pre-date this droplet (concurrent F.3.1 in-flight on `extended_tools.go` / `extended_tools_test.go` / `handler.go` per session-start git status). Confirmed unrelated by file scope: E.9 does not touch `internal/adapters/server/mcpapi`.

### Design notes

- **Package home: `internal/platform/gitenv` (NOT `internal/utils/gitenv`).** Per spec falsification mitigation #1 + CLAUDE.md "Project Structure" listing `internal/platform — OS-specific paths`, the new package fits the platform-isolation theme. `internal/utils/` does not exist in this repo and creating it would proliferate a new top-level convention. Verified the existing `internal/platform/paths.go` style (single Go file, package doc at top) and matched its conventions.
- **Single export, single concern.** `Filtered()` is the only public symbol. The package doc-comment carries the "why" (GIT_DIR override under pre-push hooks, last-wins semantics for re-added GIT_* keys); the function doc-comment carries the contract (returns fresh slice, does NOT inject any GIT_* values, callers append their own). Future additions (e.g. an `IsolatedEnv(repoRoot string)` helper that bundles the full GIT_CEILING_DIRECTORIES + GIT_CONFIG_NOSYSTEM block) would be ergonomic but out of scope; both current callers already construct their own append blocks because the GIT_CEILING_DIRECTORIES / HOME / XDG values differ between them (production points HOME at `repoRoot`; the gitdiff fixture uses a separate tempdir).
- **service.go nil-check: doc-only path chosen per spec.** Spec acceptance criterion #4 explicitly mandated keeping `nil-check + return-nil` and updating the doc-comment to document the test-injection use case. The tests would be the falsification target if I had switched to `panic("Service.gitStatusChecker not configured")` — direct-struct-literal `Service{}` constructions in tests (which exist per the spec's note "tests may inject a nil seam") would start panicking on the new gate. Document path preserves test contract; behavior unchanged.
- **Logic-equivalence verification.** Both prior helpers (`internal/app/git_status.go::filteredGitEnv` and `internal/tui/gitdiff/exec_differ_test.go::filteredEnv`) had byte-identical bodies modulo the function name: `os.Environ()` → walk → `strings.HasPrefix(e, "GIT_")` skip → append → return. Confirmed via Read of both files at session start, before any edit. The new `gitenv.Filtered()` body matches both verbatim.
- **Zero behavior change for either caller.** Both callers continue to receive `os.Environ()` minus `GIT_*` entries in the same order, same backing-store-isolation property, same per-call fresh allocation. The only observable difference is the function name in stack traces and the import line at file-top.
- **Doc-comment cross-references kept short.** Both caller-side doc-comments now point at the shared helper rather than paraphrase the round-3 motivation. The full motivation lives in `internal/platform/gitenv/gitenv.go`'s package doc-comment. This avoids a 3-way drift surface (production helper / test helper / package doc).

### Hylla feedback

None — Hylla unused this droplet. Per CLAUDE.md "Hylla Indexes Only Go Files Today" the new package + edits are all Go but the workspace is post-merge stale (HEAD `2a18010` post-dates the last ingest). Evidence resolved via `Read` + `Grep` (absolute-path) on uncommitted state, which is the correct fallback per project rules. No Hylla query was attempted because the relevant symbols (`filteredGitEnv` / `filteredEnv` / `runGitStatusPreCheck` / `defaultGitStatusChecker` / `gitFixture`) all live in files git status flagged as `M` (working-tree-modified) — Hylla would have returned stale signatures.
...
- `internal/adapters/server/common/mcp_surface.go` — added `ValidateCandidateTemplateRequest` + `ValidateCandidateTemplateResult` types and extended the `TemplateService` interface with `ValidateCandidateTemplate(ctx, req) (result, error)`. Doc-comment on `Result` documents the closed wire shape (Valid + SentinelName + Error + Warnings) and the in-band failure semantics.
- `internal/adapters/server/common/app_service_adapter_mcp.go` — added `ValidateCandidateTemplate` adapter method right after `ListBuiltinTemplates`. Plain passthrough into `app.Service.ValidateCandidateTemplate`; clones the warnings slice so the caller never sees the service-internal backing store.
- `internal/app/template_service.go` — added the `errors` import; added `ValidateCandidateTemplateInput` + `ValidateCandidateTemplateOutput` types; added the closed `templateValidationSentinels` table mapping each templates-package sentinel to its canonical Go variable name (load order matches the package-scope sentinel list); added `validationFallbackSentinelName` const for the no-sentinel-matched case; added `Service.ValidateCandidateTemplate` method that wraps `templates.LoadWithOptions` + `WarnLogger` closure capture; added `classifyTemplateValidationError` helper that walks the sentinel table via `errors.Is`.
- `internal/adapters/server/mcpapi/extended_tools.go` — added `templateValidateMaxInputBytes = 1 << 20` constant; extended `registerTemplateTools` operation enum with `validate`; added `template_toml` argument; added the 1 MiB pre-service cap; added the `case "validate":` branch that invokes the service and JSON-encodes the result via `mcp.NewToolResultJSON`. Updated the tool description to name the new operation + cap + in-band failure contract.
- `internal/adapters/server/mcpapi/extended_tools_test.go` — added the `templates` import; extended `stubExpandedService` with `lastValidateCandidateReq` / `validateCandidateCalls` / `validateCandidateResultFn` fields; added `ValidateCandidateTemplate` stub method; added `validTemplateFixtureTOML(t)` helper that re-marshals the embedded default-go template; added 5 tests: `TestTillTemplate_Validate_Valid` (stub envelope wire-shape), `TestTillTemplate_Validate_UnknownKey` (real-adapter chain → "ErrUnknownTemplateKey"), `TestTillTemplate_Validate_BadSchemaVersion` (real-adapter chain → "ErrUnsupportedSchemaVersion"), `TestTillTemplate_Validate_AgentBindingMissingWarn` (stub envelope warnings round-trip), `TestTillTemplate_Validate_OversizedRejected` (1 MiB cap; falsification mitigation #1).
- `workflow/drop_4c_5/THEME_F_PLAN.md` — flipped F.3.2 row to `**State:** done (round 1)`.

### Targets run

- `mage testPkg ./internal/adapters/server/common` — **165/165 PASS** (1.18s).
- `mage testPkg ./internal/app` — **474/474 PASS** (0.01s, cached).
- `mage testPkg ./internal/adapters/server/mcpapi` — **220/220 PASS** (1.17s). Includes the 5 new `TestTillTemplate_Validate_*` tests.
- `mage ci` — **2899/2899 PASS** across 25 packages. All coverage gates met (`internal/app` 71.6%, `internal/adapters/server/common` 72.3%, `internal/adapters/server/mcpapi` 74.1%, `internal/templates` 94.8%). Format check + tracked-sources verify + build all green.

### Design notes

- **In-band failure surface (Valid: false) chosen over transport-level tool errors.** Per spec falsification mitigation #2 + acceptance criterion #1: validation failures route through the JSON envelope's `valid` field, NOT through `toolResultFromError`. Adopters route on a uniform shape regardless of which validator inside `templates.LoadWithOptions` tripped. Transport-level `isError=true` is reserved exclusively for adapter-misconfiguration errors (nil service, oversized input, unknown JSON keys). Two of the four spec-required tests (`UnknownKey`, `BadSchemaVersion`) explicitly assert `isError=false` to lock this contract.
- **Sentinel-name extraction via closed table walked in evaluation order.** Per spec falsification mitigation #2: `templateValidationSentinels` lists each templates-package sentinel paired with its Go variable name. `errors.Is` walks the table top-down; the first match wins. Order matters because three sentinels (`ErrInvalidAgentBindingEnv`, `ErrInvalidContextRules`, `ErrInvalidAgentBindingToolGating`) all wrap `ErrInvalidAgentBinding` via `fmt.Errorf("%w: ...", ErrInvalidAgentBinding)` — the more-specific entries are listed first so they win over the generic catch-all. When no sentinel matches, `validationFallbackSentinelName = "validation_error"` is returned so the wire field is never empty on `Valid: false` results.
- **1 MiB cap enforced AT the MCP boundary, BEFORE service invocation.** Per spec falsification mitigation #1: the cap lives in `extended_tools.go` as a `len(args.TemplateTOML) > templateValidateMaxInputBytes` check. The service layer never sees an oversized payload, so allocation pressure on the templates package is bounded by the boundary. `TestTillTemplate_Validate_OversizedRejected` proves the service is NOT called when the cap is exceeded (`validateCandidateCalls == 0`).
- **Warn-logger plumbing: simple slice capture, no mutex.** `validateAgentBindingFiles` is invoked sequentially inside `LoadWithOptions` (single-threaded for-loop over `tpl.AgentBindings`) per the F.5.1 implementation, so a closure capturing into a non-shared `[]string` is race-free without a mutex. Verified via Read of `internal/templates/load.go:1388`-1417.
- **Real-adapter integration tests, NOT just stub-envelope shape.** Two of the five tests (`UnknownKey` + `BadSchemaVersion`) wire `app.NewService` + `common.NewAppServiceAdapter` to exercise the production sentinel-name extraction path end-to-end against `templates.LoadWithOptions`. The pattern matches the existing `TestActionItemMCPRejectsEmptyOrInvalidStructuralType` precedent at line ~4134. The other three tests (`Valid` + `AgentBindingMissingWarn` + `OversizedRejected`) use the stub because the wire-shape coverage they verify does not require chain semantics.
- **`validTemplateFixtureTOML` helper centralizes the canonical valid baseline.** The helper round-trips the embedded `default-go.toml` through `LoadDefaultTemplateForLanguage` + `MarshalTOML`. Two benefits: (1) future drops that change the embedded default's body land in this test for free without per-test fixture maintenance; (2) the fixture is provably the chain's reference shape — round-tripping it through the canonical pipeline guarantees `Valid: true` modulo any new validator the future drop adds.
- **No new domain types, no migration logic.** Surface-only: new request/result types in `common`, new I/O types in `app`, new wire-format-side cap constant. The closed templates-package sentinel set is unchanged. `LoadOptions.WarnLogger` (F.5.1's contract) is consumed unchanged.
- **Closed sentinel table drift contract documented in code.** The doc-comment on `templateValidationSentinels` names the drift contract loud: a future templates-package sentinel MUST be added to the table AND its name must match the package-scope variable name verbatim, or the MCP wire shape silently regresses to `"validation_error"` for the new failure mode. Future-drop builders adding sentinels read the doc-comment when they touch this table.

### Hylla feedback

None — Hylla unused this droplet. The workspace is post-merge stale (HEAD `2a18010` post-dates the last ingest), and every relevant Go symbol (`registerTemplateTools`, `Service.GetProjectTemplate`, `LoadOptions`, sentinel vars in `internal/templates/load.go`) lives in files that are working-tree-modified or freshly added in F.3.1 / F.5.1 — Hylla would have returned stale signatures. Evidence resolved via `Read` + `Grep` (absolute-path) on uncommitted state per CLAUDE.md "Hylla Indexes Only Go Files Today" + "Changed since last ingest" rules.

## Droplet F.3.3 — Round 1

**Date:** 2026-05-06.
**Builder:** go-builder-agent (model: opus).
**Source spec:** `workflow/drop_4c_5/THEME_F_PLAN.md` § "Droplet F.3.3 — `till.template` MCP tool: `set` operation (atomic install + re-bake)".
**State at end of round:** done.

### Files touched

- `internal/adapters/server/common/mcp_surface.go` — added `SetProjectTemplateRequest` (project_id + template_toml + Actor lease tuple) and `SetProjectTemplateResult` (set bool + project_id + bake_source + bytes_written + in-band error). Extended `TemplateService` interface with the `SetProjectTemplate` method. Doc-comments name the closed BakeSource vocabulary (`<bare-root>` | `<primary-worktree>` only — embedded fallbacks unreachable from `set`) and the in-band failure shape contract.
- `internal/adapters/server/common/app_service_adapter_mcp.go` — added `AppServiceAdapter.SetProjectTemplate` thin pass-through. Translates the wire request into `app.SetProjectTemplateInput` (Actor → UpdatedBy/UpdatedByName/UpdatedType) and flips service errors into `{Set: false, Error: err.Error()}` IN-BAND so the MCP boundary surfaces a uniform shape regardless of failure mode (validation, no-checkout, atomic-write, persist-rollback).
- `internal/app/template_service.go` — added new types `SetProjectTemplateInput` / `SetProjectTemplateOutput`, sentinel `ErrProjectHasNoCheckout`, and the `Service.SetProjectTemplate` method implementing the four-step atomic-install ordering (validate → shadow-bake → tmp+rename → swap+persist) per spec falsification mitigations F4 + F5. Imports extended to include `encoding/json` (KindCatalog encode) and `os` (filesystem write + rename + best-effort cleanup). Updated the file-header doc-comment to reflect F.3.3's landing.
- `internal/adapters/server/mcpapi/extended_tools.go` — extended `registerTemplateTools` with the `set` op: enforced 1 MiB cap pre-auth (defends against unauthenticated bulk-buffer attacks), called `authorizeMCPMutation` against the project-scoped action namespace `"project:<id>"` + action `"set_project_template"`, built the `ActorLeaseTuple` via `buildAuthenticatedMutationActor` with `allowUnguardedAgent=true` (mirrors the F.3.1+F.3.2 reads), then invoked `templatesSvc.SetProjectTemplate` and JSON-serialized the result. Shared input cap renamed `templateValidateMaxInputBytes` → `templateInputMaxBytes` (mirrors the dual-callsite reality per spec hint "extract a shared constant if helpful"). Tool description + Enum + arg list extended with `set` + `session_id` + `session_secret` + the guarded-mutation tuple fields.
- `internal/adapters/server/mcpapi/extended_tools_test.go` — added `lastSetProjectTemplateReq`, `setProjectTemplateCalls`, `setProjectTemplateResultFn` fields on `stubExpandedService` plus the `SetProjectTemplate` stub method (mirrors `ValidateCandidateTemplate` shape). Added six new tests: `TestTillTemplate_Set_HappyPath` (real-adapter chain end-to-end, asserts file lands on disk byte-identical + KindCatalogJSON re-bakes with 12 kinds), `_ValidationFailureNoWrite` (real chain, asserts NO file is created on validate-step short-circuit), `_AuthRejected` (stub auth-error injection asserts service is never called), `_RebakeFailureRollback` (stub forces in-band failure-with-rollback-path-named envelope), `_NoCheckoutPath` (real chain, project with empty repo paths asserts the `ErrProjectHasNoCheckout` message surfaces in-band), `_OversizedRejected` (1 MiB cap enforced pre-service). Added two new test helpers `newRealAdapterSetServer` (real `*app.Service` + `*autentauth.Service` + handler stack) and `issueProjectScopedSetSession` (issues a project-scoped approved session via `auth.IssueSession` + `approved_path` metadata), mirroring `handler_integration_test.go`'s real-auth pattern. Imports extended (`encoding/json`, `os`, `autentauth`).
- `workflow/drop_4c_5/THEME_F_PLAN.md` — flipped F.3.3 row to `**State:** done (round 1)`.

### Targets run

- `mage testPkg ./internal/app` — **474/474 PASS** (1.71s).
- `mage testPkg ./internal/adapters/server/common` — **165/165 PASS** (1.18s).
- `mage testPkg ./internal/adapters/server/mcpapi` — **226/226 PASS** (1.14s; +6 new tests over post-F.3.2 baseline of 220).
- `mage format` — gofumpt re-formatted `internal/app/template_service.go` (one minor whitespace nit). No semantic changes.
- `mage ci` — **2905/2905 PASS** across 25 packages. All coverage gates met (`internal/app` 70.8%, `internal/adapters/server/common` 72.1%, `internal/adapters/server/mcpapi` 74.2%, `internal/templates` 94.8%). Format check + tracked-sources verify + build all green.

### Design notes

- **Four-step atomic ordering exactly per spec.** Per spec acceptance criterion #2 + falsification mitigation F4: (a) `templates.LoadWithOptions` validates the candidate; (b) `templates.Bake` + `json.Marshal` build the SHADOW catalog in-memory (NOT a fresh filesystem walk — F5 mitigation); (c) `os.WriteFile(<dest>.tillsyn-set-<id>.tmp, body)` then `os.Rename(tmp, dest)` performs the atomic install; (d) `project.KindCatalogJSON = shadowEncoded` + `s.repo.UpdateProject(ctx, project)` swaps the catalog and persists. If (d) fails AFTER (c) landed, the on-disk file is moved aside via `os.Rename(dest, dest+".tillsyn-set-failed-<id>.toml")` so the dev sees the orphaned artifact and can recover manually. The error message names the rollback path verbatim.
- **`os.Rename` chosen over `os.WriteFile` direct.** POSIX guarantees rename atomicity to readers within the same filesystem; concurrent `set` operations serialize at the rename point (last-writer-wins). The tmp-file pattern means the destination either holds the prior content or the new content, never a half-written state, even if the writer crashes mid-WriteFile.
- **`os.MkdirAll(filepath.Dir(dest), 0o755)` before tmp-write.** A fresh project whose `RepoBareRoot` exists but whose `.tillsyn/` subdirectory does NOT yet exist (e.g. the dev pointed `RepoBareRoot` at a real worktree but never authored a template there) would hit `ENOENT` on the tmp-write without the MkdirAll. The 0o755 permission matches the standard convention for tracked config directories.
- **Best-effort tmp cleanup on early-return paths.** A `defer func() { if !renamed { _ = os.Remove(tmpPath) } }()` guards the case where Rename never runs (e.g. between WriteFile success and Rename — currently impossible because nothing else runs in between, but the defer is the durable contract for future inserted steps). After Rename succeeds the deferred Remove is a no-op (`os.IsNotExist` silently ignored).
- **In-band failure surface (Set: false) chosen over transport-level tool errors.** Mirrors the F.3.2 design: `{Set: false, Error: "..."}` for validation failures, no-checkout, atomic-write failures, and persist-rollback. Transport-level `isError=true` is reserved for adapter-misconfiguration (nil service) and the 1 MiB cap rejection (the cap is a wire-protocol constraint, not a domain error). This means adopters route on a uniform `Set` field rather than parsing two JSON schemas.
- **Auth gate at MCP boundary, NO service-level mutation guard.** Mirrors `SetProjectAllowedKinds`'s shape (`internal/app/kind_capability.go:142`): the service-level method trusts the boundary to have already authorized; service-side ergonomics (validation, atomic-rename, error wrapping) own the rest. The `withResolvedMutationActor` + `repo.UpdateProject(ctx, project)` thread the actor identity into the audit trail without re-running `enforceMutationGuard`. This is the established convention; deviating to add a second guard layer would be inconsistent and force test fixtures to wire a redundant capability lease.
- **Cap renamed `templateValidateMaxInputBytes` → `templateInputMaxBytes`.** F.3.2 introduced the cap with a single-callsite name; F.3.3 added the second callsite (set), so the rename reflects the dual-callsite reality. The pre-rename name was a private, single-line constant — the rename is mechanical and surfaced via doc-comment ("Pre-F.3.3 the constant was named templateValidateMaxInputBytes"). No external callers to break.
- **`pickMutationAuthorizer(templatesSvc)` re-uses the existing dispatch.** Type-asserting `templatesSvc` (which is a `common.TemplateService`) into `common.MutationAuthorizer` works because `AppServiceAdapter` (the production concrete type) implements both interfaces. The pattern is identical to `pickMutationAuthorizer(kinds)` at the `set_project_allowed_kinds` op around line 1827. No new auth-resolver plumbing required.
- **Real-adapter integration tests for the filesystem-write path.** Three of the six new tests wire `app.NewService` + `autentauth.NewSharedDB` + `common.NewAppServiceAdapter` so the actual `os.WriteFile` + `os.Rename` execution is exercised end-to-end. The happy-path test asserts both the on-disk file content AND the freshly-baked `KindCatalogJSON` contains 12 kinds (proves shadow-bake-then-swap landed). The validation-failure-no-write test asserts the on-disk file is NEVER created when validate aborts. The no-checkout test asserts the in-band error message names "no checkout". The remaining three tests use the stub because they exercise wire-shape plumbing only (auth-rejection, in-band rollback envelope, oversize cap).
- **`newRealAdapterSetServer` + `issueProjectScopedSetSession` test helpers.** `seedApprovedProjectSession` (initial design via `CreateAuthRequest` + `ApproveAuthRequest`) failed because `app.NewService` without `ServiceConfig.AuthRequests` returns "auth requests are not configured" on `CreateAuthRequest`. Pivoted to the simpler shape from `handler_integration_test.go:newRealMCPAttentionHandlerForTest`: instantiate `*autentauth.Service` directly via `NewSharedDB(autentauth.Config{DB: repo.DB()})` + `EnsureDogfoodPolicy`, wire it into `ServiceConfig.AuthBackend` + `NewAppServiceAdapter(svc, auth)`, and issue sessions directly via `auth.IssueSession(...)` with `Metadata{"approved_path": ...}` carrying the project scope. This sidesteps the auth-request audit-trail flow entirely; the resulting session is approved-equivalent for `authorizeMCPMutation`'s purposes.
- **Snapshot policy (Drop 3 5.B.14) preserved.** Per spec falsification mitigation F1: in-flight action items already created continue to use the prior catalog; new action items created after `set` use the new catalog. The cascade does NOT auto-migrate in-flight items. Doc-comment on `Service.SetProjectTemplate` names this loud (paragraph 4 of the function godoc) so adopters know the mid-cascade boundary.
- **Closed `ErrProjectHasNoCheckout` sentinel.** Exposed at the `app` package level so future callers (e.g. CLI commands wrapping the same service method) can `errors.Is(err, app.ErrProjectHasNoCheckout)` to route deterministically. The sentinel's message is the exact wording from spec acceptance criterion #3.

### Hylla feedback

None — Hylla unused this droplet. Per the spawn-prompt directive ("NO Hylla calls"), evidence-gathering used `Read` + `Grep` (absolute-path) on uncommitted state, `git diff HEAD` for sibling-droplet WIP, and `LSP`-style symbol confirmation via direct file reads. The workspace is post-merge stale (HEAD `2a18010` post-dates the last ingest), and every relevant symbol I touched lives in files that are working-tree-modified by F.3.1 / F.3.2 — Hylla would have returned stale signatures. Filesystem-MD coordination mode was the right call.

## Droplet F.3.3 — Round 2

**Date:** 2026-05-06.
**Builder:** go-builder-agent (model: opus).
**Source spec:** `workflow/drop_4c_5/THEME_F_PLAN.md` § "Droplet F.3.3 — `till.template` MCP tool: `set` operation (atomic install + re-bake)".
**Round-1 falsification finding:** `workflow/drop_4c_5/BUILDER_QA_FALSIFICATION.md` § "Droplet F.3.3 — Round 1" Attack 2 (CONFIRMED): `TestTillTemplate_Set_RebakeFailureRollback` in `extended_tools_test.go` is name-only — it stubs the in-band failure shape via `setProjectTemplateResultFn` and never reaches the production rollback branch in `Service.SetProjectTemplate` at `internal/app/template_service.go:494-511`. The `os.Rename(dest, failedPath)` path with the `.tillsyn-set-failed-<id>.toml` suffix had zero coverage at the service layer.
**State at end of round:** done.

### Files touched

- `internal/app/template_service_test.go` — **NEW**. Adds two service-level tests exercising the production rollback branch end-to-end:
  - `TestService_SetProjectTemplate_RollbackOnUpdateProjectFailure` — single-failure rollback path. Wraps `*fakeRepo` with `*errOnUpdateProjectRepo` (embedding pattern: every other Repository method delegates to the embedded fake unchanged; only `UpdateProject` is overridden to return a fixed sentinel error). Pins `idGen` to `"rb-001"` so the failed-path suffix is deterministic. Asserts (1) error is non-nil and contains the `"tillsyn-set-failed-"` substring, (2) `errors.Is(err, persistErr)` wraps the original failure, (3) the original `dest` file is absent on disk (via `os.Stat` + `os.IsNotExist`), (4) the rollback file at `dest + ".tillsyn-set-failed-rb-001.toml"` exists with byte-identical original template content, (5) exactly one rollback artifact matches the canonical glob (no orphan duplicates), (6) no `.tmp` file leaked from the WriteFile→Rename sequence.
  - `TestService_SetProjectTemplate_RollbackRenameAlsoFails` — double-failure branch (`template_service.go:502-507`). Pre-creates a non-empty DIRECTORY at the deterministic failed-path location (`dest + ".tillsyn-set-failed-df-002.toml"` with a child `blocker` file inside) so `os.Rename(dest, failedPath)` returns ENOTDIR/EISDIR/EEXIST. Asserts the louder error envelope: contains `"ROLLBACK ALSO FAILED"` sentinel substring + `"manual cleanup required"` adopter-facing instruction + still wraps the original `persistErr` via `errors.Is`.
  - Two test-private helpers: `errOnUpdateProjectRepo` (embeds `*fakeRepo`, overrides `UpdateProject`) and `seedProjectWithCheckout` (writes one `domain.Project` with `RepoBareRoot` populated into the embedded fake's `projects` map). Plus `newServiceWithRepo` constructor wrapper that pins clock + idGen for deterministic suffix prediction.
  - `minimalValidTemplateTOML` constant — copied verbatim from `internal/templates/load_test.go § TestLoadValidTemplate`. Single fixture, two tests; future load.go validator additions land as a single fixture update here rather than per-test.
- `workflow/drop_4c_5/BUILDER_WORKLOG.md` — this round-2 entry.

### Targets run

- `mage testFunc ./internal/app TestService_SetProjectTemplate_RollbackOnUpdateProjectFailure` — **1/1 PASS** (1.52s).
- `mage testFunc ./internal/app TestService_SetProjectTemplate_RollbackRenameAlsoFails` — **1/1 PASS** (1.44s).
- `mage testPkg ./internal/app` — **476/476 PASS** (1.75s; +2 over the F.3.3 round-1 baseline of 474).
- `mage formatCheck` — clean.

### Design notes

- **Embedding `*fakeRepo` instead of implementing `Repository` from scratch.** `app.Repository` is a 40+ method interface; round-1 falsification specifically called out the gap as a service-level concern and recommended `~30 LOC test`. Implementing the full interface from scratch would be ~400 LOC. The embedding pattern (`type errOnUpdateProjectRepo struct { *fakeRepo; updateProjectErr error }`) lets every existing fake method satisfy the interface unchanged; only `UpdateProject` is overridden. The override receiver matches the interface signature exactly so Go's method-promotion rules pick the wrapper's method over the embedded fake's. Verified via successful compile + green test run.
- **Deterministic id-suffix via pinned `idGen`.** The production code at `template_service.go:469-472` uses `s.idGen()` to build `tmpSuffix`, falling back to `clock().UnixNano()` if `idGen` returns empty. Test pins `idGen` to a fixed string (`"rb-001"` and `"df-002"`) so the failed-path suffix is computable up-front: `dest + ".tillsyn-set-failed-rb-001.toml"`. Without this pin, the test would have to glob the parent directory and pick the single match — reachable but flakier and weaker (the deterministic assertion catches a regression that swaps the suffix format silently).
- **Double-failure trigger via blocker directory.** `os.Rename(file, existing-non-empty-dir)` returns an error on every POSIX platform (macOS: `ENOTDIR`/`EEXIST`; Linux: `ENOTDIR`/`EEXIST`/`EISDIR` depending on glibc version). Writing a child file inside the blocker directory ensures non-empty status — empty-directory rename behavior is platform-specific (some kernels permit overwrite-empty-dir-with-file; non-empty is the safe choice). The test does NOT depend on the specific errno; it asserts only on the error STRING from the production code's `fmt.Errorf("...; ROLLBACK ALSO FAILED at %s → %s: %v; manual cleanup required", ...)` which surfaces a stable substring regardless of the underlying syscall errno.
- **Six observables on the single-failure test (vs. the spec's three) for falsification resilience.** The spawn prompt's required assertions were (a) dest absent, (b) rollback file present with original bytes, (c) error names sentinel substring. Adding (d) `errors.Is(err, persistErr)` proves the wrap is `%w` not `%v`; (e) the glob-count = 1 catches a regression where MkdirAll runs against a stale `.tillsyn` containing a prior failed file; (f) tmp-file absence catches a regression where the deferred `Remove` is skipped and a `.tmp` orphan leaks. None of these are scope-creep — they all attack the SAME branch (production rollback) but at distinct contract surfaces. Total test body remains under the spawn-prompt's "~30 LOC" guidance modulo whitespace.
- **No production code changes.** The spawn-prompt explicitly forbade touching `template_service.go`'s rollback branch — round-1 verified the branch is correct, only the test was missing. Verified by post-run `git status --porcelain internal/app/template_service.go` returning empty (no modification line). Round-2 lands tests-only.
- **`package app` (not `package app_test`).** The test file imports `projectTemplateDir` and `projectTemplateFilename` (unexported package-level constants in `internal/app/service.go:438,443`) when calling `seedProjectWithCheckout` to compute the destination path. Test-side same-package access keeps the test mirroring exactly what the production code does at `template_service.go:442,445`. Round-1 falsification's REFUTED Attack 6 already established that these constants are right-scoped as package-internal for `package app` callers.

### Hylla feedback

None — Hylla unused this round. Per the spawn-prompt directive ("NO Hylla calls"), evidence came from direct `Read` of `internal/app/template_service.go` (uncommitted, working-tree-modified) + `Grep` for the existing `fakeRepo` shape in `internal/app/service_test.go` (261k file, too big to read whole — `rg` for the relevant method signatures was the only tractable path). The workspace is post-merge stale (HEAD `2a18010` post-dates the last ingest) AND `template_service.go` carries uncommitted F.3.3-round-1 changes — Hylla would have returned doubly-stale signatures. Filesystem-MD coordination mode was the right call.
