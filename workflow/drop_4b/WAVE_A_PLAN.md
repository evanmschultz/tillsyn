...
- **Acceptance:**
  - New `type gateFunc func(ctx context.Context, input GateInput) GateResult` in `gates.go`. `GateInput` carries `Item domain.ActionItem`, `Project domain.Project`, `Template templates.Template` (or just the gate-relevant subset — TBD by builder; surface to plan-QA). `GateResult` carries `Success bool`, `Output string` (last 100 lines OR last 8KB whichever shorter, captured), `Err error`.
  - New `type gateRunner struct` with private fields: `registry map[templates.GateKind]gateFunc`, `mu sync.RWMutex` (Register/Run safety; registry mutation is constructor-time but defense-in-depth).
  ...
  - `func (r *gateRunner) Register(kind templates.GateKind, fn gateFunc) error` — rejects duplicate registration with new sentinel `ErrGateAlreadyRegistered`; rejects unregistered kind values via `templates.IsValidGateKind`.
  - `func (r *gateRunner) Run(ctx context.Context, item domain.ActionItem, project domain.Project, tpl templates.Template) (GateRunOutcome, error)` — the main entry point. Reads `tpl.Gates[item.Kind]`; iterates in declared order; on first failure halts and records `failedGate string`, `output string` in returned `GateRunOutcome`. On all-success, returns `GateRunOutcome{Success: true}`.
  - `GateRunOutcome` struct: `Success bool`, `FailedGate templates.GateKind` (empty on success), `Output string` (failure output capture, empty on success — successful gates record nothing per L1's "successful gates record nothing; only failures are noisy"), `Err error` (infrastructure error, distinct from gate-determined failure).
