...
**PASS.** All 16 required proof checks satisfied with file:line evidence. The pure dotted-address resolver in `internal/app/dotted_address.go` (113 LOC) implements the project-LESS, 0-indexed positional contract from PLAN.md `:285-294` exactly: `func ResolveDottedAddress(ctx context.Context, repo Repository, projectID, dotted string) (string, error)` at `:52`; sentinel errors `ErrDottedAddressNotFound` and `ErrDottedAddressInvalidSyntax` declared as top-level `errors.New`-style vars at `:18-21`; no `ErrDottedAddressAmbiguous` (whole-tree `rg` confirms zero hits); body regex `^\d+(\.\d+)*$` at `:26`; slug-prefix shorthand `<slug>:<dotted>` parsed and verified against the supplied `projectID` via `repo.GetProject(ctx, projectID).Slug == slug` at `:64-81`; project NEVER parsed from the dotted body (the body regex rejects any non-digit leading character, and project comes only from the function parameter); 0-indexed positions per level (`indices[i] = strconv.Atoi(seg)` then `idx >= len(children)` check at `:99-105` — `0` resolves the first child). The `Repository` interface gains `ListActionItemsByParent(ctx context.Context, projectID, parentID string) ([]domain.ActionItem, error)` at `internal/app/ports.go:33`; SQLite implementation at `internal/adapters/storage/sqlite/repo.go:1435-1460` uses `WHERE project_id = ? AND parent_id = ? ORDER BY created_at ASC, id ASC` (NOT `position ASC`) — backed by index `idx_action_items_project_parent` declared at `repo.go:490`. `fakeRepo.ListActionItemsByParent` extension at `internal/app/service_test.go:625-643` mirrors the SQLite contract via `sort.Slice` with `time.Time.Equal` tie-break and `ID` lexicographic fallback. `dotted_address_test.go` (227 LOC) carries 12 success cases, 6 not-found cases, 17 invalid-syntax cases, and 1 empty-projectID case — total 36 cases per spawn prompt. SQLite round-trip test `TestRepository_ListActionItemsByParent` at `repo_test.go:2615-2775` covers empty-parentID returning level-1 children, explicit-parent returning direct children only, project-isolation (cross-project `parent_id` collision blocked), leaf-with-no-children empty, unknown-parent empty, and same-`CreatedAt` UUID tie-break ordering. PLAN.md state correctly flipped to `**State:** done` at `:281`. No `ALTER TABLE` introduced (`rg` shows only pre-existing migration sites). No `tc := tc` in new test loops. No `mage install`. **`mage ci` re-run by QA: GREEN, 1431/1431 tests across 19 packages, 0 failures, all packages ≥ 70.0% coverage (sqlite 75.1%, app 71.8%, build of `./cmd/till` succeeded).** The implementation is faithful to PLAN R3-8 + R4-1 — dotted body is project-LESS positional, ordering exclusively `created_at ASC, id ASC` (NOT the column-scoped `position` field — see PLAN.md `:288` warning), no ambiguity error sentinel because the deterministic ORDER BY + UUID tie-break makes ambiguity unreachable. Cleared for Droplet 2.11 (CLI + MCP wiring).

### Required-Check Walkthrough

1. **`ResolveDottedAddress` signature.** `internal/app/dotted_address.go:52` — `func ResolveDottedAddress(ctx context.Context, repo Repository, projectID, dotted string) (string, error)`. Note: PLAN `:285` separates `projectID` and `dotted` as two `string` parameters; the implementation collapses them into one `projectID, dotted string` declaration which is semantically identical Go syntax. PASS.
2. **Sentinel errors.** `internal/app/dotted_address.go:18-21` — `var ( ErrDottedAddressNotFound = errors.New("dotted address not found"); ErrDottedAddressInvalidSyntax = errors.New("dotted address invalid syntax") )`. Top-level vars, `errors.New`-style. PASS.
3. **No `ErrDottedAddressAmbiguous`.** `rg "ErrDottedAddressAmbiguous"` across `internal/` and `cmd/` returns empty. PASS.
4. **Body regex `^\d+(\.\d+)*$`.** `internal/app/dotted_address.go:26` — `var dottedBodyRegex = regexp.MustCompile(\`^\d+(\.\d+)*$\`)`. Exact match. PASS.
5. **Slug-prefix shorthand `<slug>:<dotted>`.** `internal/app/dotted_address.go:64-81`. Splits on `:` (max 2), validates both halves non-empty, validates slug shape via `dottedSlugRegex` (`^[a-z0-9]+(-[a-z0-9]+)*$` at `:31`), then calls `repo.GetProject(ctx, projectID)` and asserts `project.Slug == slug`. Mismatch returns `ErrDottedAddressInvalidSyntax`. PASS. (Design note from worklog `:535`: deliberate choice NOT to expand `Repository` with `GetProjectBySlug` — the resolver verifies against the supplied projectID rather than looking up by slug; CLI/MCP slug→ID mapping is the 2.11 caller's concern. Sound.)
6. **Project-LESS body.** `internal/app/dotted_address.go:52` takes `projectID` as a separate parameter; the body regex `^\d+(\.\d+)*$` at `:26` rejects any non-digit leading character, so a UUID or slug in the body fails immediately at `:83`. The resolver never extracts a project component from the dotted body. PASS.
7. **0-indexed positions per level.** `internal/app/dotted_address.go:99-108`: `for level, idx := range indices { children, err := repo.ListActionItemsByParent(...); if idx >= len(children) { return ... ErrDottedAddressNotFound }; item = children[idx]; parentID = item.ID }`. Index `0` resolves the first child via `children[0]`. Test `:121` `{name: "single level zero", dotted: "0", wantID: "a"}` confirms behaviorally. PASS.
8. **`Repository.ListActionItemsByParent` signature.** `internal/app/ports.go:33` — `ListActionItemsByParent(ctx context.Context, projectID, parentID string) ([]domain.ActionItem, error)`. Exact match to spawn prompt. PASS.
9. **SQLite implementation `ORDER BY created_at ASC, id ASC`.** `internal/adapters/storage/sqlite/repo.go:1435-1460`. SQL at `:1442` reads `ORDER BY created_at ASC, id ASC`. NOT `position ASC` — verified directly. Index `idx_action_items_project_parent` on `(project_id, parent_id)` is declared at `:490`, providing the `WHERE` clause's index path; the `ORDER BY` is a small in-memory sort over the parent's children. PASS.
10. **`fakeRepo.ListActionItemsByParent` extension.** `internal/app/service_test.go:625-643`. Filters by `ProjectID == projectID && ParentID == parentID`, then `sort.Slice` with `time.Time.Equal` tie-break check (avoids monotonic-clock `==` pitfall) and `ID` lexicographic fallback. Mirrors SQLite contract. PASS.
11. **All 36 test cases present in `dotted_address_test.go`.** Counted: Success=12 (`:121-138`), NotFound=6 (`:163-169`), InvalidSyntax=17 (`:193-209`), EmptyProjectID=1 (`:228`). Total = 36. Spot-checks: valid `0` at `:121` (PASS), slug-prefix mismatch at `:208` `"wrong-slug:0"` (PASS), leading-zero accepted at `:136-137` `"00"`/`"02"` (PASS), malformed `1..2` at `:196` (PASS), level-out-of-range error at `:163-167` (level-1, level-2-under-b, level-3, level-2-under-leaf) — PASS via `errors.Is(err, ErrDottedAddressNotFound)`. Project-isolation is exercised by the SQLite round-trip test (item 12 below) rather than dotted_address_test.go — adequate because `fakeRepo`'s `ListActionItemsByParent` filters on `ProjectID` at `:628-630` and the resolver's per-segment lookup propagates the same `projectID`. PASS.
12. **Round-trip test `TestRepository_ListActionItemsByParent` in `repo_test.go`.** `internal/adapters/storage/sqlite/repo_test.go:2615-2775`. Covers: (a) empty parentID returns level-1 children only at `:2701-2717` — `[a-root-0, a-root-1]` for projectA; (b) explicit parent returns direct children only at `:2722-2738` — parent `a-root-1` returns `[a-tie-aaa, a-tie-zzz]`, no grandchildren; (c) project-isolation at `:2741-2756` — `b-cross` (projectB row with `parent_id="a-root-1"`) does NOT leak into projectA's listing; (d) leaf-with-no-children empty at `:2759-2765` (`a-leaf` returns 0 rows); (e) unknown-parent empty at `:2768-2774` (`does-not-exist` returns 0 rows); (f) tie-break ordering at `:2722-2738` — same `CreatedAt` (`base.Add(10 * time.Second)`) for `a-tie-aaa` and `a-tie-zzz`, ID-ASC tie-break selects `a-tie-aaa` at index 0. All required sub-cases covered. PASS.
13. **`mage test-pkg ./internal/app` + `mage test-pkg ./internal/adapters/storage/sqlite` + `mage ci` GREEN at HEAD.** QA re-ran `mage ci`: 1431/1431 tests across 19 packages, 0 failures, 0 skipped. `internal/app` 71.8% coverage, `internal/adapters/storage/sqlite` 75.1% coverage, both ≥ 70% threshold. Build of `./cmd/till` succeeded. Exit 0. The `mage ci` umbrella runs every package's tests, so per-package targets are subsumed. PASS.
14. **No `tc := tc` in new test loops.** `rg "tc := tc"` against `internal/app/dotted_address_test.go` and the new `TestRepository_ListActionItemsByParent` block in `repo_test.go` returns empty. The `for _, tc := range cases { t.Run(tc.name, ...) }` loops at `dotted_address_test.go:140`, `:171`, `:212` do not need it on Go 1.22+ where loop variables are per-iteration scoped. PASS.
15. **PLAN.md state-flip.** `workflow/drop_2/PLAN.md:281` reads `- **State:** done` for Droplet 2.10. PASS.
16. **No `ALTER TABLE` added.** `rg "ALTER TABLE"` across `internal/` returns only pre-existing migration sites in `repo.go:512-531, :548-550, :634, :695, :743`, `livewait/localipc/broker.go:531`, and `auth/autentauth/service.go:868-870` — all unchanged by Droplet 2.10. The new SQLite implementation at `repo.go:1435-1460` adds only a `SELECT` query, not a schema mutation. The `parent_id` column already exists from a Drop-0/1 migration at `:519`. PASS.

### Findings

None blocking. One minor observation:

- **(Minor, no action.)** PLAN.md `:282` cites `internal/adapters/storage/sqlite/repo.go:1393` as the line `alongside existing ListActionItems`. At HEAD the new `ListActionItemsByParent` method lands at `:1435`, with `ListActionItems` at `:1397`. The `:1393` cite was a planning-time reference that drifted under the +35 LOC insertion and prior repo-state edits. The placement is correct (immediately after `ListActionItems`); only the original line cite is stale post-edit. Not a code finding.
- **(Minor, no action.)** The worklog `:531` notes a one-iteration test-fixture rename (`a-c0-zzz` / `a-c1-aaa` → `a-tie-aaa` / `a-tie-zzz`) due to a digit-prefix self-contradiction in the original IDs. The fix is purely test-fixture; no production code changed. Reviewed the final fixture at `repo_test.go:2674-2675` and confirmed the lexicographic ordering is now unambiguous (`a-tie-aaa < a-tie-zzz` on the `aaa` vs `zzz` suffix only).

### Spot-Check Evidence

- **Slug-prefix verification path (PLAN `:287`).** `dotted_address.go:64-81` parses `<slug>:<body>`, validates both halves, then calls `repo.GetProject(ctx, projectID)` (an existing `Repository` method on the interface — verified at `ports.go:15`) and compares `project.Slug == slug`. This avoids expanding the interface with a `GetProjectBySlug` method that would also need a SQLite/fake implementation; the resolver acts as a verifier against an already-supplied `projectID`. Test `dotted_address_test.go:127-128` exercises `slug + ":0"` round-tripping; `:208` exercises `"wrong-slug:0"` rejection.
- **Created_at deterministic ordering (PLAN `:288`).** SQLite query at `repo.go:1442` reads `ORDER BY created_at ASC, id ASC`. The `id` tie-breaker is a UUID (globally unique across projects), so the listing is total per parent. `repo_test.go:2660` documents the test's intent; same-`CreatedAt` rows `a-tie-aaa` and `a-tie-zzz` at `:2674-2675` deterministically resolve to `[a-tie-aaa, a-tie-zzz]` in the assertion at `:2730`.
- **Sentinel error wrapping with `%w`.** All seven error returns in `dotted_address.go` use `fmt.Errorf("...: %w", ErrDotted...)` — `:57` (project id required), `:60` (empty dotted), `:69` (malformed slug-prefix), `:72` (invalid slug shape), `:76` (project lookup error — wraps the underlying repo error, not a sentinel), `:79` (slug mismatch), `:84` (invalid body), `:92` (segment not non-negative decimal), `:102` (list error wraps repo error), `:105` (level/idx out of range with `ErrDottedAddressNotFound`). All Sentinel-targeting wraps allow `errors.Is(err, ErrDotted...)` to work — confirmed by tests using `errors.Is` at `:174` and `:215`.
- **Body regex correctness against shape failures.** `dottedBodyRegex` `^\d+(\.\d+)*$` rejects: empty (must have `\d+`); leading dot (`.1` — first segment must be `\d+`); trailing dot (`1.` — `(\.\d+)*` requires digits after the dot); double dot (`1..2` — `\.\d+` requires digits between dots); leading dash (`-1` — `\d` excludes `-`); embedded whitespace (`1. 2` — whitespace not in `\d`). All 17 invalid-syntax test cases at `:193-209` correspond to these regex/shape failures. The regex anchors `^` and `$` ensure no leading/trailing extras.

### Acceptance-Criteria Trace

- PLAN `:285` "Function signature": MATCH at `dotted_address.go:52`.
- PLAN `:286` "Dotted body is project-LESS and 0-indexed": MATCH (no project parameter parsed from body, `children[idx]` is 0-indexed).
- PLAN `:287` "CLI shorthand `<proj_slug>:<dotted>`": MATCH at `:64-81` (slug parsed, regex-validated, slug-vs-projectID-slug equality checked).
- PLAN `:288` "Repository.ListActionItemsByParent ordered by created_at ASC, id ASC": MATCH at `repo.go:1442`. NOT confused with column-scoped `position` field (`idx_action_items_project_column_position` at `:487` is for Kanban TUI arrangement and is NOT used here).
- PLAN `:289` "SQLite implementation uses an indexed query (`WHERE project_id = ? AND parent_id = ?`); no per-call full-table scan": MATCH — index `idx_action_items_project_parent` at `:490` covers the `WHERE` clause; only the per-parent in-memory `ORDER BY` sort runs.
- PLAN `:290` "Returns the resolved action-item UUID on match": MATCH at `:111` `return item.ID, nil`.
- PLAN `:291` "Returns `ErrDottedAddressNotFound` when any level's index is out-of-range": MATCH at `:104-106`.
- PLAN `:292` "Returns `ErrDottedAddressInvalidSyntax` when input fails body shape check (or slug-prefix shape check)": MATCH at `:57, :60, :69, :72, :79, :84, :92`.
- PLAN `:293` "No `ErrDottedAddressAmbiguous` error": MATCH (whole-tree `rg` confirms absent).
- PLAN `:294` "Table-driven tests cover: valid `0`, `0.0`, `2.5.1`, slug-prefixed valid, slug-prefix mismatch, out-of-range path at each level, malformed inputs (empty, `1.`, `.1`, `1..2`, `abc`, leading-dash, deep nesting), leading-zero accepted (`007` → 7), UUID input rejected": MATCH — every enumerated case has a corresponding test entry in `dotted_address_test.go`. (The plan says `2.5.1` and the test uses `2.5.0` for the three-level case — both target the same shape and the test choice corresponds to actual fixture depth where `c-c5-g` is at `2.5.0`. Per-character literal mismatch is not a failure of the criterion which says "valid `2.5.1` (three-level)" — the criterion is satisfied by ANY in-range three-level path. `2.5.0` IS in range; `2.5.1` would be a not-found case. Acceptable interpretation.)
- PLAN `:295` "`mage test-pkg` greens": MATCH (whole-CI green).
