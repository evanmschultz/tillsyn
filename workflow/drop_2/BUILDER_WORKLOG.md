...
- `internal/app/dotted_address.go` — +96 lines (helpers + sentinel). Added: package-level `IsLikelyDottedAddress(s string) bool` shape gate, `SplitDottedSlugPrefix(dotted string) string` slug-prefix extractor, `ResolveActionItemID(ctx, repo, projectID, idOrDotted)` UUID-or-dotted dispatcher, `ValidateActionItemIDForMutation(id)` mutations-require-UUID gate, plus `*Service.GetProjectBySlug` and `*Service.ResolveActionItemID` thin wrappers. New sentinel error `ErrMutationsRequireUUID`. Imports `github.com/google/uuid`.
- `internal/app/dotted_address_test.go` — +145 lines. Added 4 new test functions covering 26 sub-cases: `TestIsLikelyDottedAddress` (17 cases), `TestSplitDottedSlugPrefix` (5 cases), `TestValidateActionItemIDForMutation` (6 cases), `TestResolveActionItemID_*` (3 cases). Coverage of helpers + sentinel is now exhaustive.
- `internal/app/ports.go` — +1 line. Added `GetProjectBySlug(context.Context, string) (domain.Project, error)` to the `Repository` interface, immediately after `GetProject` for natural reading order.
- `internal/app/service_test.go` — +9 lines. Extended `fakeRepo` with `GetProjectBySlug` mirroring the SQLite contract: O(N) scan over `f.projects` matching on `Slug`, returns `ErrNotFound` on miss. Without this, every test that constructs `fakeRepo` (25+ sites) compile-fails when the interface gains the new method.
- `internal/adapters/storage/sqlite/repo.go` — +13 lines. Added `GetProjectBySlug` method on `*Repository` immediately after `GetProject`. Query: `SELECT ... FROM projects WHERE slug = ? AND id != ?` (excluding the hidden `globalAuthProjectSlug` row). Powered by the existing `idx_projects_slug` unique index — single-row lookup, no scan.
- `internal/adapters/storage/sqlite/repo_test.go` — +49 lines. Added `TestRepository_GetProjectBySlug` round-trip covering: (a) known slug returns matching project; (b) unknown slug returns error; (c) `globalAuthProjectSlug` is hidden from this surface even when supplied verbatim.
- `internal/adapters/server/common/mcp_surface.go` — +2 lines. Extended `ActionItemService` interface with `ResolveActionItemID` + `GetProjectBySlug` so MCP handlers reach the resolver through the service contract rather than back-channeling to a private repo handle.
- `internal/adapters/server/common/app_service_adapter_mcp.go` — +30 lines. Added `(*AppServiceAdapter).ResolveActionItemID` and `(*AppServiceAdapter).GetProjectBySlug` adapter methods routing through `mapAppError` so app/domain errors land on the right transport sentinel.
- `internal/adapters/server/common/app_service_adapter.go` — +6 lines. Extended `mapAppError` to map `app.ErrDottedAddressNotFound → ErrNotFound` and `app.ErrDottedAddressInvalidSyntax | app.ErrMutationsRequireUUID → ErrInvalidCaptureStateRequest`.
- `internal/adapters/server/mcpapi/extended_tools.go` — +90 lines net. Added `app` import. Added `resolveActionItemIDForRead` helper (UUID-vs-dotted dispatch, slug-prefix MCP path, project_id-required hint) and `rejectMutationDottedActionItemID` mutation gate. Wired the gate into `case "update"`, `"move"`, `"move_state"`, `"delete"`, `"restore"`, `"reparent"` (6 mutations). Wired the resolver into `case "get"`. Updated the `till.action_item` tool description to document UUID-or-dotted on get + mutation rejection. Legacy aliases (`till.update_task`, `till.move_task`, etc.) inherit the gate via the shared `handleActionItemOperation` closure.
- `internal/adapters/server/mcpapi/extended_tools_test.go` — +210 lines net (replacement of `"t1"` literals + 12 new sub-tests + 2 helpers). Added `testActionItemUUID` constant, `ResolveActionItemID` + `GetProjectBySlug` stub methods on `stubExpandedService`, two new test functions (`TestHandlerActionItemGetAcceptsDottedAddress` with 4 sub-cases, `TestHandlerActionItemMutationsRejectDottedAddress` with 6 sub-cases), and helpers `mustNewHandler` + `errorTextFromResult`. Replaced 18 `"action_item_id": "t1"` literals with `testActionItemUUID` so the existing mutation tests survive the new validator gate.
- `cmd/till/main.go` — +94 lines net. Added `actionItemCommandOptions` struct, `actionItemOpts` variable in `main`, `actionItemCmd` parent + `actionItemGetCmd` + 6 mutation gate cobra subcommands (`update|move|move_state|delete|restore|reparent`). Threaded `actionItemOpts` through `runFlow` → `executeCommandFlow` (added param + dispatch cases). Added `actionItemCmd` to `rootCmd.AddCommand`.
- `cmd/till/action_item_cli.go` — NEW (114 lines). Implements `runActionItemGet` (UUID bypass + slug-prefix or `--project` flag + dotted resolver flow + JSON output), `runActionItemMutationGate` (validator-only stub returning the canonical `ErrMutationsRequireUUID` for dotted input or a not-yet-implemented hint for valid UUIDs), `resolveActionItemProjectContext` (slug-prefix-vs-`--project` reconciliation), and `writeActionItemJSON` (indented JSON renderer).
- `cmd/till/action_item_cli_test.go` — NEW (191 lines). 11 sub-tests across 2 test functions: `TestRunActionItemMutationGate` (4 sub-cases covering dotted reject / slug-prefix reject / UUID-passes-gate / empty-input) and `TestRunActionItemGet` (7 sub-cases covering UUID bypass / dotted with `--project` / slug-prefix shorthand / bare-without-project-errors / slug-prefix-vs-flag conflict / malformed input). Uses an in-memory SQLite + real `app.Service` + a tiny tree (root → child) for end-to-end coverage of the CLI dispatch.
- `workflow/drop_2/PLAN.md` — Droplet 2.11 `**State:** todo` → `**State:** in_progress` at start; flipped to `**State:** done` at end of round.

**Mage targets run:**

- `mage testPkg ./internal/app` — green. 279 tests passed (was 245; +34 new).
- `mage testPkg ./internal/adapters/storage/sqlite` — green. 72 tests passed (was 71; +1 new).
- `mage testPkg ./internal/adapters/server/common` — green. 140 tests passed (no new tests added in this package — the adapter additions are exercised through the MCP boundary tests).
- `mage testPkg ./internal/adapters/server/mcpapi` — green. 105 tests passed (was 93; +12 new).
- `mage testPkg ./cmd/till` — green. 220 tests passed (was 208; +12 new).
- `mage ci` — green. 1490 tests across 19 packages, 0 failures, 0 skipped. Coverage threshold met across every package (minimum 70.0%; `internal/tui` at 70.0%, `internal/app` at 71.8%, `cmd/till` at 76.5%, `internal/adapters/server/mcpapi` at 72.8%, `internal/adapters/server/common` at 72.7%). Build of `./cmd/till` succeeded. Drop 2 boundary verified.
- **Test count delta:** 1431 (post-2.10) → 1490 (post-2.11), +59 new tests across 4 packages.

**Test cycle iterations:** one round of fixup. After wiring `rejectMutationDottedActionItemID` into the 6 mutation cases, 6 existing tests in `extended_tools_test.go` failed because they passed `"t1"` (neither UUID nor dotted) into mutation operations — the new validator rejects it as not-UUID. Two changes brought them green: (a) wrapped `app.ValidateActionItemIDForMutation`'s error under `common.ErrInvalidCaptureStateRequest` so the boundary maps it to `invalid_request:` instead of `internal_error:`, and (b) introduced the `testActionItemUUID` constant and replaced every mutation-input `"t1"` literal with it. Get-path tests were also updated even though the stub's `ResolveActionItemID` echoes input — the wider fix is structural.

**Design notes:**

- **MCP slug-prefix path uses tasks.GetProjectBySlug, then forwards the slug-bearing dotted form to ResolveActionItemID.** The resolver in 2.10 already handles `<slug>:<body>` and re-validates the slug against the supplied projectID — passing the slug-bearing form through preserves that double-check. If the slug-prefix-resolved projectID's slug ever drifts from the supplied slug (caller bug or stale cache), the resolver catches it.
- **MCP get with bare dotted body requires `project_id`.** The tool description now documents this: dotted form is project-LESS by spec, and MCP's auth-gated session does NOT carry a structured project handle today (the auth context exposes session_id / session_secret only). Adding session→project lookup at the MCP boundary would mean reaching across an architectural seam (auth → app project) that's currently traversed only inside the service. Pragmatic call: require `project_id` arg for bare dotted, accept slug-prefix as the no-project-arg path. The slug-prefix gives the caller a clean substitute for "project inferred from session" without hijacking auth.
- **Mutation gate is one-line per case.** All 6 mutations get the same `if err := rejectMutationDottedActionItemID(actionItemID); err != nil { return toolResultFromError(err), nil }` immediately after the empty-string guard. Legacy aliases inherit it for free because they route through the shared `handleActionItemOperation` closure. The CLI shares the validator via `app.ValidateActionItemIDForMutation`; the CLI mutation commands today are validator-only stubs that return a not-yet-implemented hint for valid UUIDs.
- **`app.ResolveActionItemID` vs `Service.ResolveActionItemID`.** Two layers: the package-level function takes a `Repository` directly (unit-testable with `fakeRepo`); the service method is a thin wrapper that supplies `s.repo` (transport-callable through the adapter). This mirrors the existing pattern for `ResolveDottedAddress` — package-level for tests, service-method for callers.
- **`GetProjectBySlug` on the `Repository` interface, not in `ListProjects` filtering.** The CLI/MCP slug-prefix and `--project` flows both need slug→ID lookup. Doing it via `ListProjects` + linear scan would be O(N) per call and pollute every caller with the filter loop. Adding the method to `Repository` is the smaller diff (one method definition + one SQLite implementation + one fakeRepo extension). The SQLite implementation uses the existing `idx_projects_slug` unique index — single-row lookup, no scan. The hidden `globalAuthProjectSlug` is excluded by `id != ?` in the WHERE clause.
- **CLI scope reduction (acceptance #5 honored conservatively).** The droplet spec implies the CLI mutation commands exist today — but they don't (no `till action_item update`, `move`, etc. before this droplet). Two options: (a) implement the full mutation pipelines (huge scope expansion), or (b) implement validator-gated stubs that satisfy the rejection acceptance without growing the CLI surface. Took (b). The 6 mutation subcommands accept their UUID arg, validate it against `ValidateActionItemIDForMutation`, and return either the canonical mutations-require-UUID error (for dotted input) or a "not yet implemented in CLI; use the MCP surface" hint (for valid UUIDs). This satisfies acceptance #5 — `till action_item update 2.1 ...` returns the same error class as MCP — without adding mutation-flow surface area that belongs to a future drop. Surfaced explicitly to the orchestrator below.
- **`cmd/till/action_item_cli.go` is a new file, not an extension of `main.go`.** Modeled after `cmd/till/project_cli.go`. Keeps `main.go` from growing further (already 162k LOC); future CLI mutation flows attach naturally to this new file.
- **`writeActionItemJSON` over a human-readable detail view.** The `runActionItemGet` output is indented JSON. Adding a laslig-style human renderer for action items would be a separate scope concern; future drops can upgrade. JSON is the smallest surface that satisfies "shows the action item" — operators can pipe to `jq` for structured access.
- **`testActionItemUUID = "11111111-1111-1111-1111-111111111111"`.** Deterministic, parses as UUID, no semantic meaning. Replaces the legacy `"t1"` literal across every mutation test in `extended_tools_test.go`. Stub return-side `ID: "t1"` values are unaffected — the stub's response body is independent of what the test passes IN.
- **`mapAppError` extension over a transport-side handler.** New error sentinels (`ErrDottedAddressNotFound`, `ErrDottedAddressInvalidSyntax`, `ErrMutationsRequireUUID`) join the existing wrapping table. `not_found` for resolver-not-found, `invalid_request` for shape failures and mutation rejection. The `errors.Join(common.ErrInvalidCaptureStateRequest, app.ErrMutationsRequireUUID)` pattern is identical to how `domain.ErrInvalidRole` was wired in Droplet 2.5 — strict reuse, no new error class.
- **No `tc := tc` in `for _, tc := range cases`** loops in any new test file. Go 1.22+ scopes loop variables per iteration; modern style omits the shadow.
- **MCP tool description rewrite.** Existing description was preserved + extended. Dotted-form documentation lives in BOTH the tool-level description AND the `action_item_id` field description so LLM agents reading the tool surface see the rule whether they're scanning the tool list or inspecting individual fields.
- **Doc comment style.** Package-level resolver/validator comments use the same fuller-exposition style as `ResolveDottedAddress` (input contract, error taxonomy, intended call site). Adapter methods use period-terminated short summaries matching the existing surrounding methods.

**Scope-expansion surfaces reported (per agent rules):**

1. **CLI subtree creation.** The droplet spec said "CLI `till action_item get` accepts both forms" but the entire `till action_item` cobra subtree did NOT exist before this droplet — `cmd/till/main.go` shows the existing top-level commands (`serve | mcp | auth | project | embeddings | capture-state | kind | lease | handoff | export | import | paths | init-dev-config`) had no action-item peer. Built the parent + 7 subcommands fresh. Surfaced.
2. **`actionItemCommandOptions` plumbing through `executeCommandFlow`.** The flow function takes ~20 typed opt-struct params; adding the new opts as a 21st was the smallest change matching the existing pattern. No refactor of the flow signature into a struct/builder shape — that would be a refactor outside this droplet's scope.
3. **CLI mutation commands return a not-yet-implemented hint.** As discussed in Design Notes above, the spec assumed the mutation pipelines were already wired. They're not, and wiring them is several drops of work. The minimal-scope solution lands the validator gate without the mutation pipelines. **Open question for orchestrator: is this acceptable, or should I expand scope to wire at least one CLI mutation (e.g. `till action_item update`) end-to-end?** Recommending the validator-only stub stays — the rejection acceptance is met, and CLI mutation pipelines should land as a coherent batch rather than smuggled into a Drop 2 droplet that's about resolver wiring.
4. **`mapAppError` extension.** Spec said "use the same `mapAppError` pattern Droplet 2.5 set up for `ErrInvalidRole`" — done, but adding 3 new error sentinels (rather than reusing one) is a minor scope expansion. The taxonomy split (`not_found` for not-found, `invalid_request` for shape/mutation-rejection) maps cleanly to existing transport sentinels and is the right shape.

**PLAN.md state confirmation:** Droplet 2.11 flipped `todo` → `in_progress` at start of work, then `in_progress` → `done` after `mage ci` green. **Drop 2 is now complete** — all 11 droplets in `done` state.

## Hylla Feedback

None — Hylla answered everything needed.

The investigation surface for this droplet was almost entirely repo-local, named-symbol lookup: MCP handler structure (`handleActionItemOperation` closure at `extended_tools.go:857-1330`), `ActionItemService` interface (`mcp_surface.go:554`), `mapAppError` taxonomy (`app_service_adapter.go:602`), `Repository.GetProject` (`ports.go:15`), CLI command structure (`main.go:431` `runFlow` plumbing, `:1670` `rootCmd.AddCommand`), and the existing `dotted_address.go` from Droplet 2.10. Every site was named explicitly in the spawn prompt or discoverable via `git grep` on a precise symbol — Hylla symbol-search would have been redundant. No stale-ingest concerns (working in `main/`), no schema gaps, no ergonomic gripes encountered for this round.
