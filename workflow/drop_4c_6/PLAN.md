...
- **L2 sub-planner spawn directive:** "Decompose `till init` into atomic droplets per `~/.claude/agents/go-planning-agent.md` § Atomic Droplet Sizing. Likely shape: D1 vendor `fsatomic` (separate package, no `cmd/till` collision); D2 vendor `configmerge`; D3 `cmd/till/init_cmd.go` skeleton + flag wiring + JSON mode parser; D4 TUI walk (group picker + project name) using existing bubbletea infrastructure; D5 file-copy + `.gitignore` ensure (uses fsatomic — `blocked_by` D1); D6 `.mcp.json` optional registration (verify shape via Context7 against Claude Code's expected schema); D7 project-DB record creation; D8 remove `init-dev-config` from `main.go` + `help.go` + caller-audit `main_test.go`. Wire `blocked_by` between droplets sharing `cmd/till/init_cmd.go` (D3, D4, D5, D6, D7 — serial chain) AND between droplets sharing `cmd/till/main.go` (D3 [register init], D8 [remove init-dev-config] — order: D3 first since D8 only removes; safer than reverse). The `internal/vendor/fsatomic` + `internal/vendor/configmerge` droplets are NEW packages so they parallel-run with everything in `cmd/till` until the file-copy droplet (D5) needs them."
...
- **Acceptance (L1 contract; L2 plan refines):**
  ...
  - Post-render validator at `render.Render` (or a new sibling) errors out if `<bundle>/plugin/agents/<name>.md` is empty, is the OLD stub shape, or is missing `name` / `description` frontmatter.
  - Doc-comment corrections at `render.go:307-319` + `SPAWN_PIPELINE.md:24-31` reflect `--bare`-collapsed isolation per `RESEARCH/ISOLATION_ENFORCEMENT_FIX.md` §D.5 (the prior research's "two-paths model" framing was misleading without a `--bare` qualifier; current argv shape is correct — no new flags).
...
- **L2 sub-planner spawn directive:** "Decompose D.1-D.5 into atomic droplets per `~/.claude/agents/go-planning-agent.md` § Atomic Droplet Sizing. Likely shape: D1 propagate `SystemPromptTemplatePath` through `BindingResolved`; D2 3-tier agent-body resolver in `render.go:assembleAgentFileBody`; D3 frontmatter strip wiring (calls into W0's helper); D4 defense-in-depth env vars in `cli_claude/env.go`; D5 post-render validator + minimal sentinel-style assertion in `render_test.go`; D6 doc-comment corrections at `render.go:307-319` + `SPAWN_PIPELINE.md:24-31`. Wire `blocked_by`: D1 → D2 (D2 reads the field D1 plumbs); D2 → D3 (D3 mutates the body D2 produces); D2 → D5 (D5 validates the body D2 produces); D4 parallels D1-D3 (different file `cli_claude/env.go`); D6 parallels everything (doc-only)."
...
- **State:** todo
- **Kind:** `build` (atomic droplet; `Irreducible: true`)
- **Paths:** `internal/templates/builtin/till-go.toml` (RENAMED from `default-go.toml`), `internal/templates/embed.go` (//go:embed list + `LoadDefaultTemplateForLanguage` switch + `BuiltinTemplateNames`), `internal/templates/embed_test.go`, plus caller-audit edits (string literal updates only) at `internal/app/auto_generate_steward.go`, `internal/app/service.go`, `internal/app/template_service.go`, `internal/adapters/server/mcpapi/extended_tools.go` per `RESEARCH/CASCADE_ENFORCEMENT_AND_CONTEXT_PRELOAD.md` §C audit (12 source files, ~30 string edits; load-bearing sites: `embed.go:34` directive, `embed.go:136-138` resolver switch, `embed.go:178` `BuiltinTemplateNames` literal, `internal/app/service_test.go:6534` fixture).
...
- **Acceptance:**
  ...
  - `git grep "default-go.toml"` returns zero hits in `cmd/`, `internal/`, top-level Go files (workflow MD references retained per `feedback_never_remove_workflow_files.md`; comments documenting the rename history retained).
  - Caller-audit edits in 4 sites: doc-comments referring to `default-go.toml` updated to `till-go.toml` where the comment is forward-looking; historical references in droplet doc-comments (Drop 4c.5 F.2.1 mentions) retained verbatim because they describe past state.
...
- **State:** todo
- **Kind:** `build` (atomic droplet; `Irreducible: true`)
- **Paths:** `internal/templates/builtin/till-gen.toml` (RENAMED from `default-generic.toml`), `internal/templates/embed.go`, `internal/templates/embed_test.go`, plus caller-audit edits at `internal/app/service.go` (Drop 4c.5 F.1.1 + F.1.2 reference `default-generic.toml`), `internal/templates/embed.go` doc-comments referring to F.2.2 / F.1.3 / `default-generic.toml`.
- **Packages:** `internal/templates`, `internal/app` (caller audit).
- **Acceptance:**
  ...
  - `git grep "default-generic.toml"` returns zero hits in `cmd/`, `internal/`, top-level Go files; workflow MD references retained.
  - `mage ci` green.
...
- **Specify (droplet-scope; inherits `SKETCH.md` § 26.W5 + § 4.4 + § 15):**
  ...
  - **RiskNotes:** Schema-level field removal from `templates.AgentBinding` is OUT OF SCOPE — would break tests + adapter contracts; defer to Drop 4c.7. This droplet ONLY edits the shipped `till-*.toml` files + placeholder agent .md frontmatter. Frontmatter strip (W3.D3) handles RUN-TIME removal of `model:` / `tools:` if any user-supplied agent .md in `<project>/.tillsyn/agents/` carries them.
  - **ContextBlocks:**
...
- **Acceptance:**
  ...
  - "Plan Down, Build Up" section LEADS the doc per `SKETCH.md` § 26.W6 ContextBlocks `constraint`(high).
...
4c.6.W1.D1    (no blockers — Wave A head, parallel with W0 + W0.5)
4c.6.W5.D1    (no blockers — Wave A head, parallel with W0 + W0.5 + W1)
...
4c.6.W2       → 4c.6.W1.D1                        (W2 copies W1's embedded files)
4c.6.W3       → 4c.6.W1.D1, 4c.6.W0               (W3 resolver consumes W1's files; W3 frontmatter strip uses W0's helper)
4c.6.W5.D2    → 4c.6.W5.D1                        (both edit internal/templates/embed.go)
4c.6.W5.D3    → 4c.6.W5.D1, 4c.6.W5.D2, 4c.6.W1.D1 (edits the renamed files + W1's placeholders)
4c.6.W6.D1    → 4c.6.W0                           (AGENTS_CONFIG.md describes W0's schema)
4c.6.W6.D4    → 4c.6.W3                           (doc reflects W3-shipped reality)
4c.6.W6.D5    → 4c.6.W6.D1, 4c.6.W6.D2, 4c.6.W6.D3 (README cites docs that must exist)
```

Wave A (parallel — no blockers): 4c.6.W0, 4c.6.W0.5, 4c.6.W1.D1, 4c.6.W5.D1, 4c.6.W6.D2, 4c.6.W6.D3.
Wave B (after Wave A): 4c.6.W2 (after W1), 4c.6.W3 (after W1+W0), 4c.6.W5.D2 (after W5.D1), 4c.6.W6.D1 (after W0).
Wave C (after Wave B): 4c.6.W5.D3 (after W5.D1+D2+W1), 4c.6.W6.D4 (after W3).
Wave D (after Wave C): 4c.6.W6.D5 (after W6.D1+D2+D3).

The wall-clock bottleneck is the W2 sub-plan (TUI + vendor + file copy + JSON mode + `init-dev-config` removal) which has no upstream dependents but is itself a multi-droplet sub-plan; W3 (5 droplets) is the second-longest chain. W0 + W0.5 sub-plans run in parallel with everything else in Wave A.
...
- **Schema-level removal of `tools` / `model` fields from `templates.AgentBinding`** — Drop 4c.7+. W5.D3 only edits the SHIPPED template files; the schema still accepts the fields.
...
### Open questions for Phase 3 discussion

These surface as plan-QA + dev-discussion items in Phase 2/3 — not blockers on this plan's first-round QA, but worth flagging upstream:

1. **W0.5 claim-vs-impl validator's known-wired set in 4c.6 (empty) vs 4c.7 (populated by W7+W8).** The validator ships in this drop but the set is empty until 4c.7 wires `ChildRulesFor` + `context.Resolve` consumers. Acceptance asserts the validator + an "unknown-consumer claim" sentinel test exist; the actual consumer additions are deferred. Plan-QA may push back: should the validator ship as a `// TODO Drop 4c.7` no-op, or run with the empty set as documented? Current acceptance specifies the latter.
2. **W2 vendoring vs `hylla-shared` repo migration.** Sketch §9.6 commits to vendoring `fsatomic` + `configmerge` as Drop 4c.6 work; future migration to `hylla-shared` is post-MVP. Plan-QA may surface: should the vendored files include a stronger "DO NOT EDIT — re-vendor from upstream" header comment beyond just `VENDOR_SOURCE.md`? Sub-planner can decide.
3. **W2 `till init-dev-config` removal — historical config-file behavior.** `cmd/till/main.go:2039 runInitDevConfig` creates the dev config file + enforces debug logging. The sketch §9.1 says this folds into `till install`. The L2 W2 sub-planner needs to verify `till install` already does or will be extended to do these things; if not, removal is premature. Plan-QA should flag this for verification.
4. **W3.D5 post-render validator stub-shape detection signature.** The validator must distinguish "bundle body is the OLD F.7.3b stub" from "bundle body is the NEW substantive content." Stub-shape match is fragile (string-match on the stub's exact body). L2 W3 sub-planner needs to settle on a robust signal — e.g., body length threshold + presence-of-`---` frontmatter + absence-of-stub-key-phrase. Plan-QA may attack the choice.
5. **W5.D3 schema-level field-removal deferral.** `[agent_bindings.<kind>] tools` and `model` fields in `templates.AgentBinding` STAY in the schema after this drop, even though the shipped `till-*.toml` files won't set them. A future drop (likely Drop 4c.7 or later) is the natural home for the schema-level removal — but the sketch doesn't explicitly assign it to a specific later drop. Plan-QA / dev-discussion should land the deferral target.
6. **W6.D2 `CASCADE_METHODOLOGY.md` skeleton depth.** The sketch §14.1 calls for a "skeleton" but doesn't define minimum depth. Plan-QA may push: how complete is "skeleton enough"? Current acceptance specifies "1-3 paragraphs per section + `<!-- TODO populate post-dogfood -->` markers." If plan-QA wants more / less, the sub-planner can adjust.
