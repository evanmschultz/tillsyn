...
**Droplet:** `4c.6.W2.D7` — Project-DB record creation + Laslig success message + D3a `*rootCommandOptions` pointer fix.
**Round:** 1
**Date:** 2026-05-11
**Pass / Fail:** **PASS-WITH-NIT** — no CONFIRMED counterexample found; one NIT recorded (`AutoSeedStewardAnchors` divergence from production-caller pattern). Final droplet of Drop 4c.6.

### Attacks attempted

| # | Attack                                                                    | Verdict           | Detail |
| - | ------------------------------------------------------------------------- | ----------------- | ------ |
| 1 | D3a pointer fix gap — value-capture re-introduced down the chain          | MITIGATED         | Chain traced: `newInitCommand(stdout, &rootOpts)` (ptr) → RunE `*rootOpts` (deref-snapshot) → `runInitJSON(...,opts,...)` (by-value) → `runInitPipeline(...,opts,...)` → `createProjectDBRecord(ctx,opts,name)`. The pointer is only load-bearing at the closure capture boundary (cobra writes flag values into `*rootOpts` BEFORE RunE fires). Subsequent by-value passes carry the post-parse snapshot. By-value passes after the deref do NOT re-introduce the bug. `main.go:1881` confirms `&rootOpts` (not `rootOpts`). |
| 2 | Service-init pattern drift vs `executeCommandFlow` (main.go:2179+)        | MITIGATED         | Simplified service init in `createProjectDBRecord` (lines 472-515) skips config loading, autent, embeddings, live-wait — none required for a single-shot `CreateProject` call. Uses `platform.DefaultPathsWithOptions` + `os.MkdirAll(filepath.Dir(dbPath))` + `sqlite.Open` + `app.NewService` with `AutoCreateProjectColumns: true`. Path resolution matches main.go's pattern modulo `DevMode` (D7 leaves dev-mode unset — appropriate; install-mode is per-project, not dev). |
| 3 | DB-connection leak                                                        | MITIGATED         | `defer func() { _ = repo.Close() }()` at line 494. Closed deterministically on every return path. |
| 4 | Idempotency race between `ListProjects` and `CreateProject`               | ACCEPTED-AS-RISK  | Theoretical TOCTOU window exists, but `till init` is a synchronous human-invoked CLI command; no realistic concurrent invocation scenario. DB-layer UNIQUE constraint on project name would surface as a wrapped error from `Service.CreateProject` if race fires. |
| 5 | Project-name validation (empty / slash / null-byte / leading whitespace)  | ACCEPTED-AS-RISK  | `validateInitPayload` (lines 679-695) rejects empty / whitespace-only names via `strings.TrimSpace`. No further charset sanitization. Special-char handling delegated to `Service.CreateProject` / `domain.NewProjectFromInput`. Single-user CLI surface — NIT-tier; not a D7 regression. |
| 6 | HOME isolation gap across all 21 tests                                    | MITIGATED         | All `run(...)`-invoking tests set `t.Setenv("HOME", tmp)` BEFORE invocation. TUI-model-direct tests (`TestRunInitTUI_*`) don't invoke `run` so HOME doesn't bind. No test writes to dev's real `~/.tillsyn-init/`. Verified line-by-line across init_cmd_test.go. |
| 7 | Pre-existing DB schema mismatch                                           | ACCEPTED-AS-RISK  | `sqlite.Open` wraps schema-migration; mismatch surfaces as a wrapped error from line 490-492. Test always uses `t.TempDir()` so fresh DB always. |
| 8 | CONSUMER-TIE contract scope on new tests                                  | MITIGATED         | Both new tests (`TestInit_CreatesProjectRecord` line 963, `TestInit_SuccessMessage_Format` line 1009) invoke `run(context.Background(), []string{"--app", "tillsyn-init", "init", "--json", ...}, ...)` end-to-end. Pre-existing tests updated similarly. |
| 9 | Partial-success: `createProjectDBRecord` interleave with `.gitignore`     | ACCEPTED-AS-RISK  | DB creation runs AFTER `ensureGitignore`. If DB op fails, files/gitignore remain; re-run is idempotent (file-skip + DB-name-check). Acceptable shape — user sees wrapped error, can rerun. |
| 10 | Laslig message-key drift                                                  | MITIGATED         | `writeCLIKV` keys at lines 451-458: `project name`, `group`, `agents dir`, `agents copied`, `agents.toml`, `.gitignore`, `.mcp.json`, `project DB`. Test asserts `project name`, `group`, `agents copied`, `added`, `skipped`. `agents copied`'s value is `"added=N skipped=M"` — substrings `added` and `skipped` match cleanly. |
| 11 | `mage ci` re-verify                                                       | MITIGATED         | Independently ran `mage ci`: 3091 tests passing, 0 failures, all 26 packages ≥ 70.0% coverage, `cmd/till` at 76.1%, build SUCCESS. Builder's claim verified. |
| 12 | Test residue (test artifacts left in `cmd/till/`)                         | MITIGATED         | `git status --short cmd/till/` post-`mage test-pkg`: only the 3 declared modifications (`init_cmd.go`, `init_cmd_test.go`, `main.go`). No probe files, no caches. |
| 13 | Backward-compat: D5/D6 stub-error string leftovers                        | MITIGATED         | `git grep -n "project-DB record creation not yet wired" cmd/till/` → ZERO matches. `git grep -n "D7-stub\|D7 stub\|not yet wired (W2.D7)" cmd/till/` → ZERO matches. All pre-existing D5/D6 tests successfully migrated from "expect D7 stub" to "expect nil + Laslig-substring". |
| 14 | W2 container + D7 state-flip                                              | MITIGATED         | `DROP_4c.6.W2_TILL_INIT/PLAN.md:3` (`**State:** done`) ✓; line 196 D7 row `**State:** done` ✓. |
| 15 | Hylla feedback context                                                    | MITIGATED         | Builder reported `enrichment still running`. I confirmed independently — Hylla returned empty for `resolveRuntimePaths`, `autoSeedSteward`. Fell back to direct `Read`. See Hylla Feedback below. |
| 16 | Pointer dereference depth — `*rootOpts` inside closure on stable cobra state | MITIGATED         | The dereference `*rootOpts` at line 88/90 happens during RunE execution; cobra has already written flag values to `rootOpts` via PersistentFlags binding (`main.go:508-513`). Snapshot is correct at that hop. |
| 17 | `createProjectDBRecord` visibility / package boundary                     | MITIGATED         | Function is lowercase (unexported), in `package main` — appropriate scope; not designed for cross-package reuse. |
| 18 | `AutoSeedStewardAnchors` config divergence (NIT-D7R1-1)                   | NIT               | See FF/NIT section. |

### Findings

- **NIT-D7R1-1** — `cmd/till/init_cmd.go:496-498` `app.NewService(repo, uuid.NewString, nil, app.ServiceConfig{AutoCreateProjectColumns: true})` SETS `AutoCreateProjectColumns: true` but does NOT set `AutoSeedStewardAnchors: true`. Production-caller path (`cmd/till/main.go:2349-2363`) sets BOTH. Per `internal/app/service.go:80-85` docstring: "Defaults false so existing tests that pre-allocate a fixed ID slice are not retro-broken; **production callers (cmd/till/main.go) explicitly opt in.**" Per `internal/app/service.go:358-367`, the `s.autoSeedSteward` check is nested INSIDE the `s.autoProjectCols` gate, so columns are created but STEWARD anchors are NOT seeded. Result: a project created via `till init` will lack `STEWARD persistent anchor parents`. Per `internal/app/auto_generate_steward.go:30-31`, the auto-generator NEVER fabricates the anchor inline — anchor-dependent flows will fail with `errStewardParentNotSeeded` when run against a `till init`-created project. **Severity:** pre-cascade dogfood phase mitigates user-facing impact (the cascade flow is still pre-cascade per `CLAUDE.md`); but the omission is undocumented and inconsistent with the explicit "production callers opt in" contract. Recommendation: either (a) opt into `AutoSeedStewardAnchors: true` in `createProjectDBRecord` to match production-caller pattern, OR (b) add a comment explaining why D7 deliberately diverges. NIT-tier; not a release blocker.

### Hylla Feedback

- **Query:** `hylla_search_keyword` for `resolveRuntimePaths`, `AutoSeedStewardAnchors`, `autoSeedSteward`; `hylla_search` (vector+keyword) on `AutoSeedStewardAnchors steward anchor`.
- **Missed because:** `enrichment still running for github.com/evanmschultz/tillsyn@main` returned on the vector search; keyword searches on the symbols returned empty results despite the symbols existing in committed code (`internal/app/service.go:85`, `internal/app/auto_generate_steward.go`). The keyword search appears not to surface symbols inside the `app` package despite `internal_mode=include_internal` being the default. Builder hit the same enrichment stall (per worklog).
- **Worked via:** Direct `Read` on `internal/app/service.go` lines 1-380 and `internal/app/auto_generate_steward.go` lines 1-60 to verify the `AutoSeedStewardAnchors` gating logic. Direct `Read` on `cmd/till/main.go` lines 2160-2380 to verify the production-caller pattern. `git diff cmd/till/main.go` to verify the one-line pointer fix. `mage ci` for end-to-end verification (3091 tests / 26 packages ≥ 70.0% / build SUCCESS).
- **Suggestion:** (a) HEAD-aware partial-results fallback when enrichment is in-flight — return the latest fully-ingested snapshot rather than blocking the entire query; (b) when keyword search returns empty for a known-existing symbol, surface the search-mode hint ("try `id_search_mode=exact_full_id`" or "enrichment in progress — N% complete").

### Notes

- D7 is the FINAL droplet of Drop 4c.6. PASS-WITH-NIT verdict means Drop 4c.6 closes pending: (a) NIT-D7R1-1 disposition (opt-in or doc-comment), (b) drop-end close-out (Hylla reingest, MD rollup).
- The `*rootCommandOptions` pointer fix is correct and confined to `main.go:1881` — `newInstallCommand` already takes `*rootCommandOptions` (D9 doc-scrub) and that line already had `&rootOpts`. The D7 fix brings `newInitCommand` into parity.
- The simplified service init (no autent, no embeddings, no live-wait broker) is the right scope for a one-shot project-creation call. The orchestrator-driven `executeCommandFlow` service is for full runtime; D7's service is for project creation only. The only divergence worth flagging is `AutoSeedStewardAnchors` (NIT-D7R1-1).
- `mage ci` independent re-run: 3091 tests passing (builder's claim verified). `cmd/till` 76.1% coverage (builder's claim verified). 0 failures.
