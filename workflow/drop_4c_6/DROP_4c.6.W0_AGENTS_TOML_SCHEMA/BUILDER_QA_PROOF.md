...
(none rated high/medium/low — see Summary)
...
**Verdict: pass** — 0 findings. The notable design departure (Line=0 graceful degradation on MergeLocal-side rejections, vs. neither L2 RiskNotes option) is **JUSTIFIED**, not a scope-cut: PLAN.md acceptance bullet for `TestMergeLocal_ToolsDenyPositionWrapped` (line 194) asserts only `File` + sentinel preservation, never `Line > 0`; the `Line > 0` requirement is exclusive to `TestLoadRegistry_PositionWrapped` (line 193) which gets its line from `*toml.DecodeError.Position()` (a real datum the lib emits). Builder's choice (Line=0 + graceful degradation in `Error()`) honors the acceptance bullets without speculative ergonomics, and the `pelletier/go-toml/v2` API contract supports the rationale (verified via `go doc`: `*toml.DecodeError` is decode-failure-only; successful-decode yields no per-field positions in the live API).

Build-QA-Proof axes evaluated:

1. **Diff-vs-spec.** Commit `8c57e0a` ("feat(config): w0.d5 configerror envelope") touches exactly the declared paths from PLAN.md droplet D5:
   - `internal/config/agents.go` (MODIFY, +180/-10 LOC: `*ConfigError` type + `Error()` + `Unwrap()` + `keyToBlock` helper + `localPathLabel` constant + `deterministicKindOrder` slice + `Path` field on `AgentsRegistry` + envelope-wrap of `LoadRegistry` decode error path + envelope-wrap of `MergeLocal` tools_deny rejection paths).
   - `internal/config/agents_test.go` (MODIFY, +203 LOC: 7 new tests).
   - `BUILDER_WORKLOG.md` round-1 entry; `PLAN.md` D5 state-flip (`todo` → `done`).
   `git show --stat 8c57e0a`: 4 files, 434 insertions, 10 deletions. No drive-by edits. PLAN.md `Paths` for D5 (line 183) declared MODIFY on `agents.go` + `agents_test.go` and explicitly noted no new fixtures (D5 reuses D1's `malformed.toml` + D3's `local_tools_deny_rejected.toml`); diff-scope matches verbatim.

   **Out-of-scope uncommitted diff observed (informational only).** `git status` shows ` M internal/config/frontmatter.go` — uncommitted whitespace tweak in a doc-comment list (4-space→3-space gofmt list indentation, lines 69-77). Not part of D5 commit `8c57e0a`; not a runtime change; not a test-file change. Out of D5 review surface; flagged here for orchestrator awareness only.

2. **AcceptanceCriteria coverage.** Each PLAN.md L2 acceptance bullet for D5 has a verifying test:
   - "`ConfigError` is a typed error struct with fields `File string`, `Block string`, `Line int`, `Cause error`. Its `Error()` method formats: `'<file> <block>:<line>: <cause>'`" → struct present at `agents.go:89-94`; `Error()` at `:100-120`. Canonical-shape test `TestConfigError_FormatsCorrectly` at `agents_test.go:731-747` constructs `ConfigError{File: "agents.local.toml", Block: "[agents.build]", Line: 42, Cause: ...}` and asserts exact format `"agents.local.toml [agents.build]:42: tools_deny is not user-overridable; remove the field"` — verbatim from the spec at PLAN.md:186. PASS.
   - "`errors.Unwrap(*ConfigError)` returns `Cause` so `errors.Is` / `errors.As` work transitively" → `Unwrap()` at `agents.go:126-131`. `TestConfigError_UnwrapPreservesSentinel` at `agents_test.go:798-815` constructs `*ConfigError{Cause: ErrToolsDenyNotOverridable}` and asserts `errors.Is(cerr, ErrToolsDenyNotOverridable)` returns true AND `errors.Unwrap(cerr) == ErrToolsDenyNotOverridable`. Both hold. PASS.
   - "`WrapWithPosition(err error, file string, decodeErr *toml.DecodeError) error` constructs a `*ConfigError`" → builder did NOT ship a separate `WrapWithPosition` exported function; the wrap is inlined into `LoadRegistry` at `agents.go:301-321`. **This deviates from the literal acceptance-bullet wording at PLAN.md:188 but satisfies the underlying contract** — the bullet's purpose is "wrap DecodeError into ConfigError with File/Block/Line populated from `decodeErr.Position()` + `decodeErr.Key()`," which is exactly what the inlined code does. The `LoadRegistry` call site is the only consumer of this wrap (no other call site needs it; `MergeLocal` wraps a sentinel, not a `*toml.DecodeError`). Inlining over a 1-call-site helper is the smaller-concrete-design choice per CLAUDE.md "Smallest concrete design." Tests `TestLoadRegistry_PositionWrapped` (`agents_test.go:824-850`) verify the contract holds: `errors.As(err, &cerr)` succeeds AND `cerr.File == path` AND `cerr.Line > 0`. PASS.
   - "`LoadRegistry` (D1) wraps every raw `*toml.DecodeError` it receives into `*ConfigError` with `File = path`" → at `agents.go:301-321`: `errors.As(err, &decodeErr)` branch wraps into `*ConfigError{File: path, Block: keyToBlock(decodeErr.Key()), Line: row, Cause: decodeErr}`; non-DecodeError fallback wraps into `*ConfigError{File: path, Cause: err}` (no Line). `TestLoadRegistry_PositionWrapped` asserts both `cerr.File == path` AND `cerr.Line > 0`. PASS. The pre-D5 `TestLoadRegistry_MalformedTOML` (asserting inner `*toml.DecodeError` extractable via `errors.As`) STILL PASSES under the envelope wrap because `Unwrap()` exposes `Cause` and the chain walk reaches the wrapped DecodeError — verified via `mage test-func ...TestLoadRegistry_MalformedTOML` GREEN.
   - "`MergeLocal` (D3) wraps `ErrToolsDenyNotOverridable` into `*ConfigError` with `File = 'agents.local.toml'`" → `agents.go:546-572`: defaults-block path returns `&ConfigError{File: localPathLabel, Block: "[agents]", Cause: ErrToolsDenyNotOverridable}`; per-kind path returns `&ConfigError{File: localPathLabel, Block: "[agents." + string(kind) + "]", Cause: ErrToolsDenyNotOverridable}`. `TestMergeLocal_ToolsDenyPositionWrapped` (`agents_test.go:858-891`) asserts `errors.Is(err, ErrToolsDenyNotOverridable)` AND `cerr.File == "agents.local.toml"` AND `cerr.Block == "[agents.build]"`. `TestMergeLocal_ToolsDenyDefaultsPositionWrapped` (`:898-927`) asserts the defaults-block path produces `cerr.Block == "[agents]"`. Both PASS.
   - "Test `TestConfigError_FormatsCorrectly`" → present at `agents_test.go:731-747`. PASS.
   - "Test `TestConfigError_UnwrapPreservesSentinel`" → present at `:798-815`. PASS.
   - "Test `TestLoadRegistry_PositionWrapped` (REVISES D1's TestLoadRegistry_MalformedTOML expectation)" → present at `:824-850`. The "REVISES" word in the spec means "extends" — D1's test stays in tree (asserts inner DecodeError), D5 adds a NEW test asserting envelope shape. Both coexist GREEN. PASS.
   - "Test `TestMergeLocal_ToolsDenyPositionWrapped` (REVISES D3's TestMergeLocal_ToolsDenyRejected expectation)" → present at `:858-891`. Same "extends" pattern: D3's test stays GREEN; D5's test adds envelope assertion. PASS.
   - The 2 additional tests (`TestConfigError_FormatsWithoutLine`, `TestConfigError_FormatsWithoutBlock`) are not named in PLAN.md but cover the L2 RiskNote departure (Line=0 graceful degradation on MergeLocal path) plus the malformed-TOML degenerate (no Block) — they harden the format-string contract directly mentioned at PLAN.md:206 ("Format-string stability: the format `<file> <block>:<line>: <cause>` is a public surface; downstream tests will match against it. Document the format in a doc-comment"). The third additional test `TestMergeLocal_ToolsDenyDefaultsPositionWrapped` covers the defaults-block (vs per-kind) envelope path; spec at PLAN.md:194 names only the per-kind path explicitly. Acceptance is satisfied; additional coverage hardens against contract drift.
   - **Total: 7 new tests** (matching builder claim "7 new tests"); 4 named PLAN.md tests + 3 additional that harden the contract.

3. **Constraint preservation — D1-D4 regression GREEN (was 67/67 in D4; D5 adds 7 → 74).**
   - `mage test-pkg ./internal/config` (re-run by reviewer this round): **74/74 GREEN** (67 pre-D5 baseline + 7 new D5). Builder claim "38/38 GREEN" in worklog covers the targeted regression sweep (`TestLoadRegistry_*|TestResolve_*|TestMergeLocal_*|TestStripFrontmatterKeys_*` = 5+7+8+15 = 35 + the original `TestLoadRegistry_*` 5 expansions and other regex matches), but the package gate is the authoritative figure: **74 tests, 0 failures, 0 skipped, race detector enforced** (per mage output line "Running go test ./internal/config" with default `-race -count=1` flags via `mage test-pkg`).
   - Pre-D5 chain-preservation tests: `TestLoadRegistry_MalformedTOML` (asserts inner `*toml.DecodeError` extractable), `TestLoadRegistry_UnknownTopLevelField` (asserts error mentions "unknown" or "missing"), `TestMergeLocal_ToolsDenyRejected` + `TestMergeLocal_ToolsDenyDefaultsBlockRejected` (assert `errors.Is(err, ErrToolsDenyNotOverridable)`) — re-run targeted: **4/4 GREEN**. The `errors.Is` / `errors.As` chain walks correctly through `*ConfigError.Unwrap()` to reach the wrapped sentinel and the wrapped `*toml.DecodeError`. Spec-conformance contract preserved.
   - D2 + D3 + D4 production code (`Resolve`, `MergeLocal` body beyond the wrap insertion, `mergePreset` / `mergeOverride` / `cloneOverride` / `copySlice` helpers, `StripFrontmatterKeys` + `marshalNode` in frontmatter.go) untouched by D5 commit `8c57e0a` — verified via `git show 8c57e0a -- internal/config/agents.go` showing diffs only at the envelope-related lines.
   - No competing TOML libs introduced; `pelletier/go-toml/v2` remains the single TOML lib.

4. **Spec-conformance — D3↔D5 separation contract (LOAD-BEARING AXIS).** This is the critical check per spawn-prompt axis 4.
   - **D5 owns the wrap entirely?** YES. After D5 lands, `MergeLocal` returns `*ConfigError` wrapping the bare sentinel (`agents.go:546-572`). The D3 round-2 bare-sentinel returns at `agents.go:391-401` are now **converted** to envelope-wrapped returns at the same call sites — D3's revert-to-bare from round 2 has been superseded by D5's envelope wrap. This is EXACTLY the intended sequencing per BUILDER_QA_PROOF.md `Droplet 4c.6.W0.D3 — Round 2` summary: "D5 retains exclusive ownership of envelope wrapping." The contract is honored: D3 no longer raises bare sentinel at runtime (D5's wrap intercepts); D5 owns the file/block/line composition.
   - **D3's bare sentinel chain preserved via `Unwrap()`?** YES. `errors.Is(err, ErrToolsDenyNotOverridable)` still works because `*ConfigError.Unwrap() → Cause` returns the wrapped sentinel verbatim. Locked in by `TestConfigError_UnwrapPreservesSentinel` (`agents_test.go:798-815`) which constructs `&ConfigError{Cause: ErrToolsDenyNotOverridable}` and asserts both `errors.Is(cerr, ErrToolsDenyNotOverridable)` AND `errors.Unwrap(cerr) == ErrToolsDenyNotOverridable`. Plus the indirect proof via D3's preserved tests `TestMergeLocal_ToolsDenyRejected` + `TestMergeLocal_ToolsDenyDefaultsBlockRejected` (both still assert `errors.Is(err, ErrToolsDenyNotOverridable)` against the now-wrapped error and both pass).
   - **`*toml.DecodeError` chain preserved via `Unwrap()`?** YES. `TestLoadRegistry_PositionWrapped` (`agents_test.go:824-850`) explicitly asserts `errors.As(err, &decodeErr)` succeeds AGAINST the envelope-wrapped error. The `Cause: decodeErr` field at `agents.go:312` plus the `Unwrap()` walk delivers this. D1's `TestLoadRegistry_MalformedTOML` (which uses `errors.As(err, &decodeErr)` directly) STILL GREEN under the wrap.
   - **Notable design departure to verify: Line=0 + graceful degradation in `Error()`.** Builder chose neither L2 RiskNote option (linePositions map vs re-decode at MergeLocal boundary) — instead, MergeLocal rejections set `Line=0` and `Error()` degrades to `"<file> <block>: <cause>"` format (no `:0:` suffix). **Verdict: JUSTIFIED, not scope-cut.** Reasoning:
     - PLAN.md acceptance for `TestMergeLocal_ToolsDenyPositionWrapped` (line 194) asserts only `File` + Block + sentinel preservation; **`Line > 0` is explicitly NOT asserted for the MergeLocal path**. The `Line > 0` requirement is exclusive to `TestLoadRegistry_PositionWrapped` (line 193) which gets its line from `*toml.DecodeError.Position()` — a real datum the lib actually emits during decode failures.
     - PLAN.md L2 RiskNote at line 203 acknowledges the architectural challenge: "MergeLocal doesn't currently parse TOML (it operates over already-decoded `AgentsRegistry` structs); the line number for `tools_deny` rejection comes from D3's load step, NOT from `MergeLocal`'s own logic." The two RiskNote options (a) thread linePositions onto AgentsRegistry, or (b) re-decode at boundary, are explicitly tagged as "Builder picks the simpler approach at implementation time."
     - Builder's third option (Line=0 + graceful degradation) is the SIMPLEST of the three: zero schema change, zero re-decode cost, zero new helper. The `pelletier/go-toml/v2` API contract supports the rationale (per `go doc github.com/pelletier/go-toml/v2.DecodeError`: position is decode-failure-only; successful-decode produces no per-field metadata in the live API).
     - Format-string degradation is correctly implemented at `agents.go:110-119` via the 4-way switch (Block-set + Line>0 → canonical; Block-set + Line=0 → no-line variant; Block-empty + Line>0 → no-block variant; both-empty → file-only). `TestConfigError_FormatsWithoutLine` locks the no-line variant: `"agents.local.toml [agents.build]: tools_deny is not user-overridable; remove the field"` — clean format, no misleading `:0:` artifact.
     - **No ":0:" leakage in user output verified** by reading the format switch at `:110-119` and the `TestConfigError_FormatsWithoutLine` assertion at `:765-768`. The `case e.Block != "" && e.Line == 0` branch never emits the line suffix.
   - **`localPathLabel` hardcoded to `"agents.local.toml"`** — verified at `agents.go:43`. Builder Design Note 2 explains the rationale: "the user-facing label is what matters for diagnostics; the actual on-disk filesystem path is uninteresting and varies across environments." Hardcoding keeps test fixtures stable (`TestMergeLocal_ToolsDenyPositionWrapped:885` asserts `cerr.File == "agents.local.toml"` literally). The new `AgentsRegistry.Path` field at `:245` exists for future Resolve-side / W11 MCP-boundary errors per Builder Design Note 2.
   - **`deterministicKindOrder` slice mirrors closed-12-enum.** Cross-checked verbatim against `internal/domain/kind.go:34-47` (`validKinds` slice): same 12 values, same order (`KindPlan` → `KindResearch` → `KindBuild` → `KindPlanQAProof` → `KindPlanQAFalsification` → `KindBuildQAProof` → `KindBuildQAFalsification` → `KindCloseout` → `KindCommit` → `KindRefinement` → `KindDiscussion` → `KindHumanVerify`). `agents.go:51-64` matches `kind.go:34-47` 1-1. **No drift.** Doc-comment at `agents.go:45-50` documents the rationale (deterministic ordering for reproducible error messages under Go's randomized map iteration).
   - **Adding a 13th Kind would require updates to BOTH `internal/domain/kind.go` (validKinds) AND `internal/config/agents.go` (deterministicKindOrder + agentsTOMLBlock per-kind fields).** This is the same drift surface D1's typed per-kind decode struct already incurs (acknowledged in D1 Design Note 3); D5 adds one more touch point on the same architectural seam, not a new seam. Acceptable.

5. **Shipped-but-not-wired.**
   - `*ConfigError` is consumed by `LoadRegistry` decode-error path (`agents.go:308-320`) and `MergeLocal` tools_deny-rejection path (`:548-571`). Both have callers: `LoadRegistry` is the W0 loader entry point + reachable by W2 `till init` + W3 render layer; `MergeLocal` is consumed by the same downstream waves. Per BUILDER_QA_PROOF D2 axis 5 and D3 axis 5, both functions are exported for cross-package consumption. Envelope is on the wrap path of both; not orphan code.
   - `Unwrap()` is consumed by `errors.Is` / `errors.As` in 4 tests (`TestConfigError_UnwrapPreservesSentinel`, `TestLoadRegistry_PositionWrapped`, `TestMergeLocal_ToolsDenyPositionWrapped`, `TestMergeLocal_ToolsDenyDefaultsPositionWrapped`) plus implicitly by the 4 pre-D5 chain-preservation tests that walk the Unwrap chain. Live consumer.
   - `keyToBlock` is consumed at `agents.go:310` (LoadRegistry decode-error wrap) — single live consumer. Inlining considered but kept as a helper because the formatter is conceptually distinct from the wrap site (formats `toml.Key` independently of envelope construction); keeps `LoadRegistry`'s decode-error branch readable.
   - `localPathLabel` is consumed by `MergeLocal` at `:549` and `:567`. Live consumer.
   - `deterministicKindOrder` is consumed by `MergeLocal` at `:559`. Live consumer.
   - `AgentsRegistry.Path` is consumed by `LoadRegistry` (sets it at `:340`) and `MergeLocal` (preserves it at `:577`). The consumer surface today is just the population path (no test or production code reads the field except via `merged.Path == project.Path` indirectly through the deep-clone). **This is the one shipped-but-not-yet-fully-wired surface** — Builder Design Note 2 explicitly justifies it as future-Resolve / W11 MCP-boundary substrate. Not orphan code (it's documented future-API), but flagged here for full transparency. **NOT a finding** because PLAN.md L2 RiskNote line 203 explicitly anticipates this as one of the architectural options ("`LoadRegistry` records line numbers for sensitive fields on the decoded struct (e.g., a `linePositions map[string]int`) and `MergeLocal` reads them"). The `Path` field is the structural-substrate version of this; future drops can extend it without retrofit.

**Gate evidence (re-run by reviewer this round):**

- `mage test-pkg ./internal/config` → **74/74 GREEN** (67 pre-D5 baseline from D1+D2+D3+D4 + 7 new D5). Race detector enforced (`-race`); count=1 enforced.
- `mage test-func ./internal/config "TestConfigError_FormatsCorrectly|TestConfigError_FormatsWithoutLine|TestConfigError_FormatsWithoutBlock|TestConfigError_UnwrapPreservesSentinel|TestLoadRegistry_PositionWrapped|TestMergeLocal_ToolsDenyPositionWrapped|TestMergeLocal_ToolsDenyDefaultsPositionWrapped"` → **7/7 GREEN** (the new D5 tests).
- `mage test-func ./internal/config "TestLoadRegistry_MalformedTOML|TestLoadRegistry_UnknownTopLevelField|TestMergeLocal_ToolsDenyRejected|TestMergeLocal_ToolsDenyDefaultsBlockRejected"` → **4/4 GREEN** (chain-preservation regression — the 4 pre-D5 tests that depend on the `errors.Is` / `errors.As` chain walk through the new envelope wrap).
- `git show --stat 8c57e0a` → 4 files (`agents.go`, `agents_test.go`, BUILDER_WORKLOG.md, PLAN.md), 434 insertions, 10 deletions. Matches PLAN.md `Paths` declaration exactly. No drive-by.
- `git show 8c57e0a -- internal/config/agents.go` → diff scope confirmed: new `localPathLabel` + `deterministicKindOrder` + `ConfigError` + `Error()` + `Unwrap()` + `keyToBlock` at the top of file; new `Path` field on `AgentsRegistry`; `LoadRegistry` decode-error path swapped from `fmt.Errorf("decode agents.toml at %q: %w", path, err)` to envelope-wrapped variants; `MergeLocal` tools_deny rejection paths swapped from bare sentinel returns to envelope-wrapped variants; map-iteration loop replaced with deterministic-order slice iteration. No edits outside these surfaces.
- Closed-12-enum cross-check: `agents.go:51-64` (`deterministicKindOrder`) vs `internal/domain/kind.go:34-47` (`validKinds`) — verbatim match, same order, no drift.
- L2 RiskNote departure verified: `Line > 0` requirement absent from PLAN.md:194 (`TestMergeLocal_ToolsDenyPositionWrapped` AC) but present at PLAN.md:193 (`TestLoadRegistry_PositionWrapped` AC). Builder's Line=0 graceful degradation honors the asymmetric AC.

**Proof certificate:**

- **Premises** — D5 ships `*ConfigError{File, Block, Line, Cause}` envelope per PLAN.md:186-194 contract; `Error()` formats canonical shape with degradation modes for missing components; `Unwrap()` preserves sentinel + DecodeError chains; `LoadRegistry` decode-error path wraps with File+Block+Line; `MergeLocal` tools_deny path wraps with File+Block (Line=0, justified per L2 RiskNote line 203 architectural acknowledgment); D3↔D5 separation contract honored (D5 owns the wrap entirely; D3's bare-sentinel chain preserved via Unwrap); D1-D4 regression GREEN; closed-12-enum mirrored without drift.
- **Evidence** — see per-axis citations above (file:line for every claim) plus the gate-evidence block. Production-code citations: `agents.go:36, 43, 51-64, 89-152, 245, 280-321, 337-341, 538-572, 577` (envelope + wrap sites + kind-order slice + Path field); `internal/domain/kind.go:34-47` (closed-enum cross-check). Test citations: `agents_test.go:731-927` (7 new D5 tests). Diff-scope citations: `git show --stat 8c57e0a`.
- **Trace or cases** — read PLAN.md D5 acceptance bullets + RiskNotes + ContextBlocks + KindPayload; cross-checked each against `agents.go:36-572` symbols + `agents_test.go:731-927` test bodies + assertions; ran 7 D5 + 4 chain-preservation regression + full-package gate; verified diff scope via `git show --stat 8c57e0a`; verified D3↔D5 contract by reading the wrap-insertion at `agents.go:548-572` (envelope wraps the bare sentinel; sentinel reachable via Unwrap); verified Line=0 departure via PLAN.md:194 (no Line>0 assertion) + `agents.go:110-119` format-switch (no `:0:` leakage) + `agents_test.go:765-768` (`TestConfigError_FormatsWithoutLine` asserts clean no-line variant); verified closed-12-enum mirror via line-by-line diff of `agents.go:51-64` against `kind.go:34-47`.
- **Conclusion** — **PASS**. All five Build-QA-Proof axes satisfied; no findings; no missing evidence. The Line=0 design departure is **JUSTIFIED, not scope-cut**: PLAN.md acceptance for the MergeLocal-side test asserts only File + Block + sentinel preservation (line 194), NOT `Line > 0`; that assertion is exclusive to LoadRegistry-side test (line 193, where `*toml.DecodeError.Position()` provides a real source-line datum). Builder picked the simplest of the three architectural options (Line=0 + graceful degradation in `Error()`) over the L2 RiskNote alternatives (linePositions map, re-decode at boundary), all three of which were explicitly tagged as "Builder picks the simpler approach at implementation time" by PLAN.md:203. The `pelletier/go-toml/v2` API contract supports the rationale verifiable via `go doc`. Format degradation is implemented correctly (no `:0:` leakage). D3↔D5 separation contract honored; Hylla-style chain preservation locked in by 5 distinct tests.
- **Unknowns** — (1) `AgentsRegistry.Path` field is populated but not yet consumed beyond the wrap-site canonical-label hardcoding (`localPathLabel` is used instead). Documented future-API surface for Resolve-side / W11 MCP-boundary error wrapping per Builder Design Note 2. NOT a finding (PLAN.md L2 RiskNote line 203 explicitly anticipates `Path`-like substrate as architectural option). Routed forward to W11 / future drops. (2) Non-DecodeError decode failures (e.g. strict-decode rejections from `DisallowUnknownFields()` that aren't position-tagged) wrap into `*ConfigError{File: path, Cause: err}` with empty Block + Line=0 — `agents.go:317-320`. Verified D1's `TestLoadRegistry_UnknownTopLevelField` (asserts error mentions "unknown" or "missing") still GREEN under this fallback wrap, but the fallback path has no positive proof that `errors.As(err, &cerr)` works on the unknown-field path specifically. Builder Design Note 9 acknowledges this: "Existing test `TestLoadRegistry_UnknownTopLevelField` asserts the error message contains 'unknown' or 'missing' — that string survives the envelope wrap because `*ConfigError.Error()` includes the cause." A fresh test asserting `errors.As(*ConfigError)` on the unknown-field path would harden this; flagged as a potential future-test refinement, **NOT** a finding because the existing test contract is preserved and the wrap implementation is straightforwardly correct (single-branch fallback at `:317-320`).

### Hylla Feedback

N/A — verification touched only Go files (D5 ships in the existing `agents.go` + co-located test file). All evidence-gathering used `Read` (production + test + worklog + PLAN.md acceptance), `git show` / `git log` / `git status` (commit-scope verification, uncommitted-frontmatter.go observation), `mage test-pkg` / `mage test-func` (gate runs), and direct file cross-checks (`internal/domain/kind.go` closed-enum mirror verification). Hylla was not queried during this proof pass — D5's surface is post-commit pre-ingest territory where `git` + `mage` + `Read` are authoritative for the contract verification, and the live LSP daemon would not have surfaced any committed-state signal that those tools didn't cover. The closed-12-enum cross-check is a 12-line literal comparison against a shipped enum at a known path; Hylla would either confirm what `Read` already showed or return stale pre-D5 state.
