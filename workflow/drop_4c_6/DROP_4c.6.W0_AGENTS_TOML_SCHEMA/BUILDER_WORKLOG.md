...
- `internal/config/agents.go` (MODIFY; +~120 LOC — new `*ConfigError` envelope type + `Error()` + `Unwrap()` + `keyToBlock` helper + `localPathLabel` constant + `deterministicKindOrder` slice + `Path` field on `AgentsRegistry` + envelope wrap of `LoadRegistry` decode error path + envelope wrap of `MergeLocal` tools_deny rejection).
- `internal/config/agents_test.go` (MODIFY; +~165 LOC — 7 new tests: `TestConfigError_FormatsCorrectly`, `TestConfigError_FormatsWithoutLine`, `TestConfigError_FormatsWithoutBlock`, `TestConfigError_UnwrapPreservesSentinel`, `TestLoadRegistry_PositionWrapped`, `TestMergeLocal_ToolsDenyPositionWrapped`, `TestMergeLocal_ToolsDenyDefaultsPositionWrapped`).
- `workflow/drop_4c_6/DROP_4c.6.W0_AGENTS_TOML_SCHEMA/PLAN.md` (state-flip W0.D5 `todo → done`; round-1 closed without an intermediate `in_progress` checkpoint because the implementation landed in a single TDD pass — no rework required).
- `workflow/drop_4c_6/DROP_4c.6.W0_AGENTS_TOML_SCHEMA/BUILDER_WORKLOG.md` (this entry).
...
- `mage test-func ./internal/config TestConfigError_FormatsCorrectly` — RED (build error: undefined `ConfigError`), then GREEN after the envelope landed in `agents.go`.
- `mage test-func ./internal/config "TestConfigError_.*|TestLoadRegistry_PositionWrapped|TestMergeLocal_ToolsDenyPositionWrapped|TestMergeLocal_ToolsDenyDefaultsPositionWrapped"` — 7/7 GREEN on first run after implementation.
- `mage test-func ./internal/config "TestLoadRegistry_.*|TestResolve_.*|TestMergeLocal_.*|TestStripFrontmatterKeys_.*"` — 38/38 GREEN (full regression sweep across W0.D1+D2+D3+D4 + new D5; no test from earlier droplets broke under the envelope wrap).
- `mage format` — clean.
