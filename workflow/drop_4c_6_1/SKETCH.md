...
| FE location | inline `tillsyn/main/fe/` for now | Same rationale. |
...
| Vim keybindings — FE/desktop (W6) | **`fe/frontend/src/lib/vim/` engine + wails-keys filter + palette** | TS-side engine inside WebView. `wails-keys.ts` filters OS-level keys (Cmd+Q/M/W/H stay native) before dispatching to engine. Migration target ro-vim (KEYBIND-R2). Vitest + Playwright tests. |
...
| Vim command palette v1 | **`:dispatch`, `:plan`, `:close`, `:archive`, `:settings`, `:help` (disposition 7.5)** | Minimum set. Future commands added as Tillsyn surfaces grow. |
