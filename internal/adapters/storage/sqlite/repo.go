...
func (r *Repository) CreateProject(ctx context.Context, p domain.Project) error {
	...
	_, err = r.db.ExecContext(ctx, `
		INSERT INTO projects(
			...
			hylla_artifact_ref, repo_bare_root, repo_primary_worktree, build_tool, dev_mcp_server_name,
			kind_catalog_json,
...
func (r *Repository) CreateActionItem(ctx context.Context, t domain.ActionItem) error {
	...
	packagesJSON, err := encodeActionItemPackagesJSON(t.Packages)
	if err != nil {
		return err
	}
	filesJSON, err := encodeActionItemFilesJSON(t.Files)
	if err != nil {
		return err
	}
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO action_items(
			id, project_id, parent_id, kind, scope, role, structural_type, irreducible, owner, drop_number, persistent, dev_gated, paths_json, packages_json, files_json, start_commit, end_commit, lifecycle_state, column_id, position, title, description, priority, due_at, labels_json,
...
func (r *Repository) UpdateActionItem(ctx context.Context, t domain.ActionItem) error {
	...
	res, err := tx.ExecContext(ctx, `
		UPDATE action_items
		SET parent_id = ?, kind = ?, scope = ?, role = ?, structural_type = ?, irreducible = ?, owner = ?, drop_number = ?, persistent = ?, dev_gated = ?, paths_json = ?, packages_json = ?, files_json = ?, start_commit = ?, end_commit = ?, lifecycle_state = ?, column_id = ?, position = ?, title = ?, description = ?, priority = ?, due_at = ?,
...
func scanActionItem(s scanner) (domain.ActionItem, error) {
	...
	t.Paths = paths
	packages, err := decodeActionItemPackagesJSON(packagesRaw)
	if err != nil {
		return domain.ActionItem{}, err
	}
	t.Packages = packages
	files, err := decodeActionItemFilesJSON(filesRaw)
	if err != nil {
		return domain.ActionItem{}, err
	}
	t.Files = files
	...
	// EndCommit mirrors StartCommit: raw TEXT column, no JSON encoding,
	// pre-droplet-4a.9 rows (and the NOT NULL DEFAULT '' invariant) read
	// as the empty string, the legitimate "not yet captured" zero value.
	t.EndCommit = endCommitRaw
...
// encodeActionItemPackagesJSON serializes the Packages slice for storage in
// the action_items.packages_json column. Nil/empty slice encodes to "[]" so
// the schema's NOT NULL DEFAULT '[]' invariant is preserved on every write.
// Per Drop 4a droplet 4a.6 the storage shape is JSON-encoded text, mirroring
// the labels_json / paths_json precedent — packages is always read whole on
// action-item reads, individual-package queries are dispatcher-side in-memory
// after the read, so SQLite JSON1 functions are unnecessary.
func encodeActionItemPackagesJSON(packages []string) (string, error) {
	if len(packages) == 0 {
		return "[]", nil
	}
	bytes, err := json.Marshal(packages)
	if err != nil {
		return "", fmt.Errorf("encode packages_json: %w", err)
	}
	return string(bytes), nil
}

// decodeActionItemPackagesJSON deserializes packages_json into []string.
// Empty or whitespace-only raw payloads decode to nil (zero-value Packages)
// so legacy rows or NULL-coerced reads do not surface as decode errors.
// JSON null also decodes to nil.
func decodeActionItemPackagesJSON(raw string) ([]string, error) {
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" || trimmed == "null" {
		return nil, nil
	}
	...
	if err := json.Unmarshal([]byte(trimmed), &packages); err != nil {
		return nil, fmt.Errorf("decode packages_json: %w", err)
	}
	return packages, nil
}

// encodeActionItemFilesJSON serializes the Files slice for storage in the
// action_items.files_json column. Nil/empty slice encodes to "[]" so the
// schema's NOT NULL DEFAULT '[]' invariant is preserved on every write.
// Per Drop 4a droplet 4a.7 the storage shape is JSON-encoded text,
// mirroring the labels_json / paths_json / packages_json precedent — files
// is always read whole on action-item reads, individual-file queries are
// consumer-side (Drop 4.5 file-viewer) in-memory after the read, so SQLite
// JSON1 functions are unnecessary.
func encodeActionItemFilesJSON(files []string) (string, error) {
	if len(files) == 0 {
		return "[]", nil
	}
	bytes, err := json.Marshal(files)
	if err != nil {
		return "", fmt.Errorf("encode files_json: %w", err)
	}
	return string(bytes), nil
}

// decodeActionItemFilesJSON deserializes files_json into []string. Empty or
// whitespace-only raw payloads decode to nil (zero-value Files) so legacy
// rows or NULL-coerced reads do not surface as decode errors. JSON null
// also decodes to nil.
func decodeActionItemFilesJSON(raw string) ([]string, error) {
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" || trimmed == "null" {
		return nil, nil
	}
