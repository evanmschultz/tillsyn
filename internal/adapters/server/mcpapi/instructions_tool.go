...
func recommendedInstructionSettings() []string {
	return []string{
		...
		"State explicitly that failing tests, CI, or QA require a new follow-up task or subtask before repair work begins, including after a previously completed item needs more fixes.",
		...
		"When documenting refactor metrics, say that builders update the slice task description with git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings after QA or validation, and that orchestrators roll those values up to the parent phase description plus the report artifact.",
...
func recommendedMDFileGuidance() map[string][]string {
	return map[string][]string{
		"AGENTS.md": {
			...
			"Workflow truthfulness policy: no implementation, cleanup, QA, parity-check, or repair work happens without an explicit task or subtask, and failing tests/CI/QA create a new follow-up item before repair.",
			...
			"Refactor metrics policy: builders update slice task descriptions with git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings, and orchestrators roll those totals up into the parent phase description plus the report artifact.",
		},
		"CLAUDE.md": {
		...
		"README.md": {
			...
			"Document the failure-repair rule explicitly: failing tests, CI, or QA require a new follow-up item before repair work begins, including after a previously completed item needs more fixes.",
			...
			"Document that refactor metrics belong in the slice task description and parent phase description as well as the report artifact, and name the expected categories such as git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings.",
