...
func recommendedInstructionSettings() []string {
	return []string{
		...
		"State explicitly that no implementation, cleanup, QA, parity-check, or repair work should happen without an explicit task or subtask at the correct level.",
		"State explicitly that failing tests, CI, or QA require a new follow-up task or subtask before repair work begins, including after a previously completed item needs more fixes, and say that Tillsyn does not auto-create that repair item today.",
		"Document phase-level push-and-reingest confirmation as part of the shipped default workflow rather than as a refactor-only special case.",
		"When documenting the shipped default templates, call out refactor-phase, dogfood-refactor-phase, refactor-task, and dogfood-refactor-task as first-class kinds and explain their metrics expectations truthfully.",
		"When documenting refactor metrics, say that builders update the slice task description with git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings after QA or validation, that orchestrators roll those values up to the parent phase description plus the report artifact, and that Tillsyn does not auto-verify every metric field or rollup total today.",
...
func recommendedMDFileGuidance() map[string][]string {
	return map[string][]string{
		"AGENTS.md": {
			...
			"Workflow truthfulness policy: no implementation, cleanup, QA, parity-check, or repair work happens without an explicit task or subtask, and failing tests/CI/QA require a new follow-up item before repair that the orchestrator or human must create explicitly because Tillsyn does not auto-create it today.",
			...
			"Refactor metrics policy: builders update slice task descriptions with git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings, and orchestrators roll those totals up into the parent phase description plus the report artifact; Tillsyn generates those checkpoints but does not auto-verify every field or rollup total today.",
		},
		"CLAUDE.md": {
		...
		"README.md": {
			...
			"Document phase-level push-and-reingest confirmation as part of the default workflow and explain that shipped refactor and dogfood-refactor kinds also carry metrics expectations.",
			"Document that refactor metrics belong in the slice task description and parent phase description as well as the report artifact, name the expected categories such as git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings, and say that Tillsyn does not auto-verify every field or rollup total today.",
