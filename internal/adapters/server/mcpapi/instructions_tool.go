...
func recommendedInstructionSettings() []string {
	return []string{
		...
		"Treat actionItem/project details and comment summaries/bodies as markdown content in all agent-authored payloads.",
...
func recommendedMDFileGuidance() map[string][]string {
	return map[string][]string{
		...
		"README.md": {
			...
			"State clearly that active actionItem coordination, worklogs, and execution tracking belong in Tillsyn rather than in markdown files.",
			...
			"Document that refactor metrics belong in the slice actionItem description and parent phase description as well as the report artifact, name the expected categories such as git-diff deltas, before/after repo and Hylla counts, timing windows, and cleanup/security findings, and say that Tillsyn does not auto-verify every field or rollup total today.",
			"Document that depends_on, blocked_by, and blocked_reason are the current explicit sequencing tools for actionItem/phase prerequisites until richer visual ordering rules exist.",
			"Explain where scoped rules live: project standards_markdown, template descriptions and child rules, and branch/phase/actionItem metadata such as objective, acceptance criteria, definition of done, and validation plan.",
			...
			"Markdown-first guidance for actionItem/project details and comment content.",
		...
		"MCP_DOGFOODING_WORKSHEET.md": {
			"Step-by-step validation scenarios covering actionItem, thread, and attention flows.",
