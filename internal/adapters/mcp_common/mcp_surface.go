...
// SupersedeActionItemRequest stores transport input for the supersede
// escape hatch (Drop 4c.5 droplet B.1). The supersede transitions one
// `failed` action item to `complete` with `metadata.outcome =
// "superseded"` and the supplied reason persisted on
// `metadata.transition_notes`. Two surfaces invoke this path: the
// human-only CLI (`till action_item supersede`) and the MCP tool
// (`till.action_item operation=supersede`, gated by
