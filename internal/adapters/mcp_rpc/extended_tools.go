...
func registerProjectTools(
	...
	srv.AddTool(
		mcp.NewTool(
			...
			mcp.WithString("repo_primary_worktree", mcp.Description("(required for operation=create) Drop 4a L4 first-class field. Absolute filesystem path to the project's primary worktree (e.g. \"/Users/.../hylla/tillsyn/main/\"). Required so the dispatcher can `cd` into the correct checkout when spawning subagents; omitting it breaks agent-isolation hook bootstrap. Must be absolute.")),
...
func registerCommentTools(srv *mcpserver.MCPServer, comments mcpcommon.CommentService, authContexts *mcpAuthContextStore) {
	...
	srv.AddTool(
		mcp.NewTool(
			...
			mcp.WithString("target_type", mcp.Description("project|action_item"), mcp.Enum("project", "action_item")),
