...
func TestNewHandoffNormalizesAndValidates(t *testing.T) {
	...
	handoff, err := NewHandoff(HandoffInput{
		...
		TargetScopeType: ScopeLevelActionItem,
		TargetScopeID:   " qa-actionItem ",
		TargetRole:      " QA ",
		...
		RelatedRefs:     []string{" actionItem-1 ", "actionItem-1", " actionItem-2 "},
	...
	if len(handoff.RelatedRefs) != 2 || handoff.RelatedRefs[1] != "actionItem-2" {
...
func TestHandoffUpdateTransitions(t *testing.T) {
	...
	if handoff.TargetBranchID != "branch-qa" || handoff.TargetScopeID != "actionItem-qa-2" || handoff.TargetRole != "qa" {
...
func TestNormalizeHandoffListFilter(t *testing.T) {
	...
	if filter.ProjectID != "project-1" || filter.ScopeID != "actionItem-1" {
	...
	if _, err := NormalizeHandoffListFilter(HandoffListFilter{ProjectID: "project-1", ScopeID: "actionItem-1"}); !errors.Is(err, ErrInvalidScopeType) {
