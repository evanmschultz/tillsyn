...
//
// The command's environment is built from gitenv.Filtered() — os.Environ()
// with every GIT_*=... entry stripped — and then isolation overrides are
// appended. Filtering is critical: when this test binary runs inside a
// `git push` pre-push hook, git itself sets GIT_DIR / GIT_INDEX_FILE /
// GIT_WORK_TREE / GIT_PREFIX / GIT_REFLOG_ACTION / etc. on the hook's
// environment, all pointing at the bare-root repo that invoked the hook. If
// we let those leak in via os.Environ(), git honors GIT_DIR over any
// discovery-walk logic, completely bypassing GIT_CEILING_DIRECTORIES — and
// `git init` writes to the bare-root config (which is locked by the
// in-flight push) instead of the per-test tempdir. Round 1 (config
// isolation) and Round 2 (ceiling dir) both appended to os.Environ() and so
// failed under the hook context with:
//
//	error: could not lock config file <bare-root>/config: File exists
...
//
// Drop 4c.5 E.9 moved the filter to internal/platform/gitenv.Filtered() so
// the production git_status pre-check and this test fixture share one
// implementation; the round-3 motivation above is preserved verbatim.
