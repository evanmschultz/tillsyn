...
// TestNormalizeColumnStateIDStrictCanonicalRejectsLegacyLiterals verifies the
// TUI column-name normalizer rejects legacy state literals via empty-string
...
func TestNormalizeColumnStateIDStrictCanonicalRejectsLegacyLiterals(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want string
		...
		{name: "legacy with surrounding whitespace rejected", in: " completed ", want: ""},
		{name: "custom column Backlog preserved", in: "Backlog", want: "backlog"},
		{name: "empty stays empty", in: "", want: ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := normalizeColumnStateID(tc.in); got != tc.want {
				t.Fatalf("normalizeColumnStateID(%q) = %q, want %q", tc.in, got, tc.want)
			}
		})
	}
}
