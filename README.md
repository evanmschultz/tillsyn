...
- **Cascade methodology dogfooded end-to-end across 5 cascades** (Phase 4.2 `domain.Project.Language` removal → Phase 4.3 `--language` CLI/MCP/TUI teardown → E2E-8 `wait_timeout` auth-flow fix → Phase 4.4 `LoadDefaultTemplateForLanguage` retirement → E2E-5/6/10 heterogeneous friction bundle). Each cascade exercised the recursive planner + plan-QA pair + build-QA pair + chain-tail `mage ci` pattern; see `CASCADE_METHODOLOGY.md` § "Dogfood Evidence (2026-05-15)" for the worked progression. Across 4 plan-QA-gated cascades, plan-QA caught 2 load-bearing decomposition errors that would have manifested as failed `mage ci` commits or wrong-surface fixes (Phase 4.4 ADD/MIGRATE/MIGRATE/DELETE 4-droplet restructure + E2E-6 planner-premise correction).
...
- **`domain.Project.Language` field and every `--language` CLI/MCP/TUI surface retired** through Phase 4.2 → 4.3 → 4.4. `domain.Project` is now five first-class fields (`HyllaArtifactRef`, `RepoBareRoot`, `RepoPrimaryWorktree`, `BuildTool`, `DevMcpServerName`). Phase 4.4 closed the loop by deleting `templates.LoadDefaultTemplate` + `LoadDefaultTemplateForLanguage` after migrating the `loadStewardSeedTemplate` seam to `func(project domain.Project)` with project-tier-first + `till-gen` fallback; `LoadBuiltinTemplate(name string)` is the single template-load API now.
- **E2E-5/6/10 friction bundle closed** (commits `edfa130` + `1a932e8` + `8ea3822`): `till project discover` now emits all 6 W2.D7 first-class fields; TUI project-form's root-path now prefers `project.RepoPrimaryWorktree` over the TUI cache (was reading stale `m.projectRoots[slug]`); `till_action_item operation=create` with `parent_id == project_id` now auto-clears to top-level instead of returning the confusing `not_found: authorize mutation` error.
- **`mage ci` GREEN** at HEAD across 28 packages, 3294 tests, coverage gates ≥70% everywhere. CI green on both jobs (`ci macos-latest` + `release snapshot check`).
- **Open E2E friction:** E2E-1 (TUI path editing — picker doesn't surface, typed path doesn't save) is the last open ticket. E2E-9 (`.mcp.json` global-binary scoping) closed 2026-05-15 as not-a-bug — the auth+lease system already provides project scoping at the token-binding layer regardless of which binary serves MCP. See `E2E_FIXES.md`.
...
- add better onboarding/template support for existing projects:
  - when onboarding an existing repo, one of the first project-root tasks should be to review all relevant markdown files in the bare repo and active worktree, gather rules/plans that matter, discuss with the user what should move into Tillsyn, and identify what should remain docs or be cleaned up,
  - this should become an explicit reusable template/onboarding pattern instead of an ad-hoc manual reminder,
  - and the default onboarding UX should make it obvious that markdown is intake material to review and migrate, not the long-term working backlog.
- support a non-branch execution shape when no real git branch/worktree lane is intended:
  ...
  - but dogfood/setup work should also support a phase-only execution path so operators do not get forced `BRANCH CLEANUP` work when no branch was ever created,
