...
- **Workflow-drop nesting.** The `project` is NOT a workflow drop — it is the root container. Immediate children of the project are **top-level Drops** (Drop 0, Drop 1, …). In the action-items tree this is the root `plan` node for that workflow drop. Drop 3 introduces the `metadata.structural_type` axis (`drop | segment | confluence | droplet`, waterfall metaphor) for finer classification within a workflow drop.
...
- [x] **Promote `metadata.role` to a first-class domain field.** Closed-enum values: `builder`, `qa-proof`, `qa-falsification`, `qa-a11y`, `qa-visual`, `design`, `commit`, `planner`, `research`. The `kind` enum carries the parent-context distinction (e.g., `kind=plan-qa-proof` vs `kind=build-qa-proof`); the `role` enum carries the QA flavor (proof vs falsification). Templates dispatch agents using both axes. Note: `commit`, `research`, `planner` role values are pure echoes of the same-named kind values — the dual-axis design only earns its keep on QA kinds where parent-context disambiguates; non-QA role values exist for agent-binding-lookup symmetry. Implementation: pure parser + tests in `internal/domain/role.go` (regex `^Role:\s*([a-z0-9-]+)\s*$` per line, returns first match or empty, rejects unknown values against the closed enum, ~50 LOC + table-driven tests) + `Role` field on `ActionItem` struct + SQLite schema column + MCP surface field on action-item create/update/get request and response schemas. **Pre-MVP rule: no migration logic in Go code, no `till migrate` CLI subcommand, no SQL backfill — dev fresh-DBs `~/.tillsyn/tillsyn.db` after the unit lands.** The parser exists as a domain helper for callers that want to opportunistically lift role from description prose at create time; it is not wired into a hydration runner.
- [x] **State-vocabulary rename: `done → complete` AND `progress → in_progress` (bundled in one migration).** Same SQL column, same code surfaces — running both renames in one sweep is materially cheaper than two passes. **Strict-canonical** — every legacy alias (`done`, `progress`, `completed`, `doing`, `in-progress`) is REJECTED at every coercion site, not coerced. Custom `ChecklistItem.UnmarshalJSON` rejects legacy `"done"` JSON keys. Pre-slug literal rejection in `normalizeStateID` / `normalizeStateLikeID` / `normalizeColumnStateID`.
  - Domain: `StateDone → StateComplete`, `StateProgress → StateInProgress` constants in `internal/domain/workitem.go:17-22`. `IsTerminalState` + alias normalization at `:154` flipped (existing `"complete"`/`"completed"` aliases pointing to `StateDone` get flipped to `StateInProgress` likewise).
  - **`ChecklistItem.Done bool` field at `internal/domain/workitem.go:84` renamed to `ChecklistItem.Complete bool`** including the JSON serialization key. Every entry in `templates/builtin/default-go.json` (40+ hits at lines 311-1465 carrying `"done": false`) gets rewritten to `"complete": false`. Persisted `completion_contract` rows: SQL migration script rewrites `"done":` → `"complete":` in checklist JSON payloads.
  - DB column `action_items.state`: SQL migration rewrites `'done' → 'complete'` and `'progress' → 'in_progress'`. Combined with any leftover `failed` state migration since they touch the same column.
  - TUI: `canonicalSearchStatesOrdered` at `internal/tui/model.go:305`, `searchStates` at `:1231`, `dependencyStates` at `:1236` slice constants. Label maps at `:321-322`. `internal/tui/options.go:147` default-state list. Plus any other `"done"` / `"progress"` literals — full grep sweep before commit.
  - MCP normalization at `internal/adapters/server/common/app_service_adapter_mcp.go:853,892` state-coercion switches: `"complete"`/`"completed"`/`"done"` all coerce to `StateComplete`; `"in-progress"`/`"in_progress"`/`"progress"`/`"doing"` coerce to `StateInProgress`. Alias tolerance lets pre-rename callers work; canonical writes use the new values.
  - Templates: every `templates/builtin/*.json` state value swept.
  - Docs: PLAN.md, CLAUDE.md, WIKI.md, agent files — every prose `done`/`progress` referring to lifecycle state.
  - `archived` is unchanged.
- [x] **Strip hardwired nesting defaults from the domain catalog (mechanism stays).** Any action item is nestable in any other action item DURING Drop 2; the closed 12-kind enum stays closed but every catalog row's `KindDefinition.AllowedParentScopes` is set to empty (universal-allow) at boot-seed time. The `AllowsParentScope` enforcement path at `internal/app/kind_capability.go:566` continues to call `kind.AllowsParentScope(parent.Scope)` — with empty `AllowedParentScopes` it returns true for every parent, so nesting is universally permitted but the mechanism is intact. Real reverse-hierarchy prohibitions + auto-create rules + per-project nesting constraints land in Drop 3's template overhaul (see §19.3). Drop 2 also deletes the speculative `domain.AllowedParentKinds(Kind) []Kind` function added in Drop 1.75 Unit A — that function has zero production callers (verified: only `internal/domain/domain_test.go:680-714` test fixtures + 2 doc-comments at `internal/app/snapshot.go:448` and `internal/adapters/storage/sqlite/repo.go:300`); the LIVE production path is `AllowedParentScopes` field + `AllowsParentScope` method, which Drop 2 leaves operational. Net change: ~5 sites (boot seed payloads, one DB UPDATE script, doc-comment cleanup, test fixture flip, dead-function deletion).
- [x] **Dotted-address fast-nav (CLI + MCP read paths, TUI bindings in drop 4.5)** *(§1.4 convergence landing in drop 2)*. Implement dotted-address resolution (`N`, `N.M`, `N.M.K`, `<proj_slug>:<dotted>` slug-prefix shorthand OR explicit `--project <slug>` CLI flag) as a pure resolver in `internal/app`: takes a dotted body + project context (supplied separately, NOT in the body), returns the UUID of the matching node (or `ErrDottedAddressNotFound` / `ErrDottedAddressInvalidSyntax`). Body is 0-indexed positions among parent's children at each level via `Repository.ListActionItemsByParent` ordered `created_at ASC, id ASC`. Wired into `till.action_item(operation=get)` MCP + CLI read commands. **Dotted addresses remain read-only**: 6 mutation operations (`update | move | move_state | delete | restore | reparent`) reject dotted form and require UUIDs. `create` doesn't take an existing action_item_id so the gate doesn't apply there.
- [x] **`scope` column — defer removal to a future refinement drop.** Drop 1.75 mirrored `scope` from `kind` to avoid a ~20-file Go ripple (`ActionItem` struct, `NewActionItem` validator, `DefaultActionItemScope`, `IsValidWorkItemAppliesTo`, SQL column + indexes, MCP request schemas). Drop 2 acknowledged the deferral; the column-strip lands in a future Hylla-assisted refinement drop.
- [x] **Per-drop wrap-up:** update CLAUDE.md + agent files for the `metadata.role` promotion, the bundled `done → complete` + `progress → in_progress` rename, and the universal-nesting domain default (templates own constraints in Drop 3+). *(Completed: flipped backtick-wrapped `done` → `complete` in lifecycle prose across `main/CLAUDE.md`, `main/WIKI.md`, and the 9 `~/.claude/agents/*.md` files. `~/.claude/agents/` files are not git-tracked here; tracked changes land in `main/CLAUDE.md` + `main/WIKI.md`.)*

**Order matters:** `metadata.role` first (no state-machine changes, pure metadata migration via D1+D2), then the bundled state rename `done → complete` + `progress → in_progress` (touches state machine + `failed` state column + `ChecklistItem.Complete` field + template JSON keys, all in one migration), then strip-nesting-defaults (orthogonal, can run in parallel with the rename if convenient — touches only catalog seed payloads + 2 doc-comments + the dead `AllowedParentKinds` function deletion). Dotted-address reads land last because the resolver has zero coupling to the other Drop 2 changes — letting rename-induced test churn settle first means resolver tests don't get rewritten mid-flight.

**Out of Drop 2 scope (explicit):** commit cadence rules (templates own this in Drop 3+, `commit` kind already exists from Drop 1.75); reverse-hierarchy prohibitions (closeout-no-closeout-parent, commit-no-plan-child, etc. — all live in templates per Drop 3 overhaul); auto-create rules (`build` auto-creates `build-qa-*`, etc. — also Drop 3 templates); research-as-kind acknowledgment (already a kind post-Drop-1.75, no migration); template wiring (Drop 3); dispatcher (Drop 4); TUI overhaul (Drop 4.5).

### 19.3. drop 3 — Template Configuration

**Template Overhaul Scope (expanded 2026-04-25).** Drop 3 is a full template-system overhaul, not an extension of the existing `KindTemplate` scaffolding. Reverse-hierarchy prohibitions (e.g., `closeout`-no-`closeout`-parent, `commit`-no-`plan`-child, `human-verify`-no-`build`-child, `build-qa-*`-no-`plan`-child), auto-create rules (e.g., `build` auto-creates `build-qa-proof` + `build-qa-falsification`), nesting constraints, commit cadence rules, and per-project overrides ALL live in the new template system — no domain-level carve-outs, no hardwired prohibitions. Existing `KindTemplate` + `KindTemplateChildSpec` + `AllowedParentScopes` + `AllowsParentScope` get rewritten, not extended. Architecture target: closed TOML schema at `<project_root>/.tillsyn/template.toml` with strict unknown-key rejection and a versioned `schema_version`; template baked into a `KindCatalog` value type at project-creation time (no runtime file lookups); single `Template.AllowsNesting(parent, child Kind) (bool, reason string)` function as the one validation truth; `[child_rules]` table for auto-create with load-time validator catching unreachable rules / cycles / unknown kinds; default template at `templates/builtin/default.toml` covering the conceptual prohibitions for projects that ship no template; plan-QA-falsification attack vector that pairs every (parent, child) kind combo against the validator and asserts agreement with declared rules; audit-trail `till.comment` + attention item on every nesting rejection; pure Go struct unmarshaling with no string-based DSL.

**Cascade Vocabulary Adoption (dev direction, 2026-04-17).** Before template configuration can be encoded cleanly, the node vocabulary needs a stable classification that templates can bind to. Drop 3 adopts a **waterfall metaphor** that aligns the branding (`Tillsyn Cascade`) with the node-type axis templates need. Concretely: `drop` is the level_1 cascade step (parallelizable, one bare worktree + branch + drop-orch per drop); a new `segment` concept names a parallel execution stream within a drop (fan-out); a `confluence` is a merge/integration item where multiple segments or drops converge; a `droplet` is the atomic, indivisible leaf action. These four classifications live on `metadata.structural_type` (closed enum, NOT open-ended) and are orthogonal to `metadata.role` (closed enum: see §19.2 bullet 1 for the full set). Templates `child_rules` and gate rules bind on `structural_type`. This lands at the **start of drop 3** because every bullet below depends on having a stable classification vocabulary to bind rules against.

**Commit cadence rules (added 2026-04-25).** Drop 3 owns the commit-cadence-as-template-rule design + plan + discussion. Default template ships with a sane cadence (e.g., one `commit` child per `plan` at depth ≥ 2 whose subtree contains at least one `complete` `build`). Custom templates override per-project. Edge cases the discussion converges before Drop 4's dispatcher binds the rule: multi-PR drops (one commit per plan vs one per drop), cascade-owned vs dev-owned commit, partial-success subtrees (some `build` children complete, some failed), template override patterns (`auto_commit = false` for SQL migration / scratch work plans).
...
- [ ] **Template binding by `structural_type`.** Templates bind `child_rules`, gate rules, validation constraints, and agent bindings on the `structural_type` axis. A template declares: "a `drop` auto-creates a `planner` droplet + `qa-proof` droplet + `qa-falsification` droplet"; "a build `droplet` auto-creates `qa-proof` + `qa-falsification` sibling droplets"; "a `confluence` requires non-empty `blocked_by`"; and so on. `structural_type` carries the semantic shape orthogonal to the closed 12-value `kind` enum + the closed `role` enum.
...
- [ ] **Marketplace evolution (post-Drop-4c)** *(captured during Drop 4a planning to persist scope across compactions; Drop 4c lands the MVP CLI surface; these items are post-MVP)*. Drop 4c (pre-Drop-5 polish) ships the marketplace foundation: separate `github.com/evanmschultz/tillsyn-templates` repo, CLI surface (`till template list|fetch|show|install|validate`), generic + Go + FE builtin separation, project-template auto-discovery from `.tillsyn/template.toml`, and the `till.template` MCP tool. Post-MVP refinements documented for persistence:
  - [ ] **TUI marketplace browser** (Drop 4.5+ scope; FE/TUI track). Visual template list, diff against current, install one-click, history view by commit + date.
  - [ ] **Vector search.** Marketplace repo CI precomputes `<name>.embedding.json` per template (and per template tag). Tillsyn binary downloads embeddings during `fetch`. `till template search "<query>"` runs cosine-sim locally against cached embeddings. Embedding storage in marketplace repo (NOT in Tillsyn binary) keeps embeddings updateable without binary release.
  - [ ] **User contribution flow.** GitHub PR against the marketplace repo. CI runs `tillsyn template validate --strict <path>` on each PR file. Manual review for design quality; merge auto-updates `INDEX.toml`. Eventually allow signed templates / curator review.
  - [ ] **Live-runtime validation / dry-cascade simulation.** Take a synthetic action-item tree, walk dispatcher logic against the template, assert no orphans / no infinite loops / every promotion has a binding. Heavier than Drop 4c's static checks; requires dispatcher reusability for simulation mode.
  - [ ] **Template inheritance / extends.** A project template may declare `extends = "go-cascade"` and override only specific bindings. Reduces duplication for adopter projects that follow the canonical Go cascade with one or two tweaks.
  - [ ] **Template-bound agent prompts.** Today agent prompt files are global (`~/.claude/agents/*.md`). Marketplace templates may want to ship custom agent prompts inline (e.g. a `[agent_prompts.go-builder-agent]` table). Requires sandboxing semantics + adopter trust.
  - [ ] **Versioned template references on Project.** `project.template_ref = "tillsyn-templates@v1.4.0/go-cascade"` so a project pins a specific marketplace version. Update flow: `till template update` re-fetches + re-bakes if the pinned ref hasn't moved.
- [ ] **Per-drop wrap-up:** update CLAUDE.md + agent files.
