...
- [ ] **Per-drop wrap-up:** update CLAUDE.md + agent files for the `metadata.role` promotion, the bundled `done → complete` + `progress → in_progress` rename, and the universal-nesting domain default (templates own constraints in Drop 3+).
