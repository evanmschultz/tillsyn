...
`),
		Example: []string{
			...
			"  till embeddings status --project-id p1 --limit 20",
		},
	},
...
`),
		Example: []string{
			...
			"  till embeddings reindex --project-id p1 --wait --wait-timeout 30s",
		},
	},
	"till kind": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			...
			"  till kind allowlist set --project-id p1 --kind-id task --kind-id research-task",
		},
	},
	"till kind list": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			"  till kind allowlist list --project-id p1",
			"  till kind allowlist set --project-id p1 --kind-id task --kind-id qa-check",
		},
	},
...
`),
		Example: []string{
			"  till kind allowlist set --project-id p1 --kind-id task --kind-id subtask",
...
`),
	...
	"till lease": {
		Example: []string{
			...
			"  till lease issue --project-id p1 --agent-name builder-1 --role builder",
			"  till lease renew --agent-instance-id builder-1 --lease-token lease-abc --ttl 30m",
			"  till lease revoke-all --project-id p1 --reason operator_reset",
		},
	},
	"till lease list": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			"  till lease issue --project-id p1 --agent-name builder-1 --role builder",
			"  till lease issue --project-id p1 --scope-type task --scope-id task-123 --agent-name qa-1 --role qa --requested-ttl 30m",
			"  till lease issue --project-id p1 --agent-name orchestrator-1 --role orchestrator --allow-equal-scope-delegation",
		},
	},
...
`),
		Example: []string{
			"  till lease heartbeat --agent-instance-id builder-1 --lease-token lease-abc",
		},
	},
	"till lease renew": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			"  till lease revoke-all --project-id p1 --reason operator_reset",
			"  till lease revoke-all --project-id p1 --scope-type branch --scope-id branch-1 --reason branch_recovery",
		},
	},
	"till handoff": {
		Example: []string{
			"  till handoff create --project-id p1 --summary \"Builder blocked on QA\"",
			"  till handoff get --handoff-id handoff-123",
			"  till handoff list --project-id p1 --status open",
			"  till handoff update --handoff-id handoff-123 --summary \"QA resumed\"",
		},
	},
	"till handoff create": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			"  till handoff create --project-id p1 --summary \"Builder blocked on QA\"",
			"  till handoff create --project-id p1 --scope-type task --scope-id task-123 --source-role builder --target-role qa --next-action \"re-run verification\"",
			"  till handoff create --project-id p1 --summary \"Need review\" --missing-evidence test-output --related-ref task-123",
		},
	},
	"till handoff get": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			...
			"  till handoff list --project-id p1 --scope-type task --scope-id task-123",
			"  till handoff list --project-id p1 --status open --status accepted --limit 20",
		},
	},
	"till handoff update": {
		Long: strings.TrimSpace(`
...
`),
		Example: []string{
			"  till handoff update --handoff-id handoff-123 --summary \"QA resumed\"",
			"  till handoff update --handoff-id handoff-123 --status accepted --target-role builder",
			"  till handoff update --handoff-id handoff-123 --summary \"Complete\" --resolution-note \"validated and closed\"",
		},
	},
	"till export": {
		Long: strings.TrimSpace(`
